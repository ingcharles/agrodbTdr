...
try{
	...
	$numeroOficio = htmlspecialchars ($_POST['numeroOficio'],ENT_NOQUOTES,'UTF-8');
	$observacionAsistente = htmlspecialchars ($_POST['observacionAsistente'],ENT_NOQUOTES,'UTF-8');
	...
	try {
		...
		$cti->actualizarNumeroOficioTramite($conexion, $idSolicitud, $numeroOficio);
		$cti->actualizarEstadoTramite($conexion, $idSolicitud, $estado);
		$fechaDespacho = date('Y-m-d h:m:s');
		$cti-> guardarSeguimientoTramite($conexion, $idSolicitud, $inspector, $fechaDespacho, $observacionAsistente);
