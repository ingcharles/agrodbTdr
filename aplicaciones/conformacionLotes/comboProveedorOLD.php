	...
	case 'proveedor':		
		...
		if($parametro>0){
		    ...
		    /*if($parametro['areas']==1){
		        $sitio = $cl->sitiosXidProductoAcopiador($conexion, $producto,$operador,"ACO");
		        echo '
        			<div data-linea="4" id="resultadoSitio" >
		...
		echo "<label for=proveedores>Nombre del Proveedor: </label>
		 	<select id=proveedores name=proveedores>
		 	<option value=>Seleccione....</option>";
	...
	$('#cantidad').keyup(function(e){
		...
		if(str!=""){
			...
			if(index>=0){			
				...
				console.log("entro caca");
				if($("#cantidad").val().indexOf('.')!=-1){					
			        if($(this).val().split(".")[1].length > 2){
			        	console.log("es cacao");   
...
$("#cantidad").on("blur keypress",function (event) {
	/*var val =$("#nproducto").val();
	//alert(str);
	var producto;
	var index;
	alert(val);
	if(val!=""){
		producto = val.toLowerCase();
		index = producto.indexOf("pitahaya");
		if(index>=0){				
		    $("#cantidad").val($(this).val().replace(/[^\d].+/, ""));        
		     if ((event.which < 48 || event.which > 57)) {
		         if(event.which != 8)        	 
		         event.preventDefault();
		     }
...
$("#proveedores").change(function(event){
	...
	if($.trim($("#proveedores").val())){
		...
		$("#nuevoProductoProveedor").attr('data-destino', 'areaResultado');
