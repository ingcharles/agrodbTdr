...
<form id="enviarAprobacion" data-rutaAplicacion="reformaPresupuestaria" data-opcion="enviarRevisionGF" data-accionEnExito="ACTUALIZAR" >
	<input type='hidden' id='identificador' name='identificador' value="<?php echo $identificador;?>" />
	
	<p>La siguiente <b>Reforma Presupuestaria</b> será enviada para aprobación en Gestión Financiera - DGAF: </p>
	...
	<?php
		if($elementosPlanificacionAnual != 0){
			for ($i = 0; $i < count ($elementosPlanificacionAnual); $i++) {
				...
				$presupuestosRevisados = pg_fetch_result($crp->numeroPresupuestosRevisadosTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'revisadoGA'), 0, 'num_presupuestos_revisados');
				...
				if(($planificacionAnual['estado'] == 'revisadoGA') && (($total-$presupuestosAprobados) == $presupuestosRevisados)){
					echo'<fieldset>
				...
				}else if(($planificacionAnual['estado'] == 'revisadoGA') && ($planificacionAnual['id_area_aprobador'] != null)){
					echo'<fieldset>
							<legend>Código N° </label>' .$planificacionAnual['id_planificacion_anual'].'</legend>
								<div data-linea="1">
									<label>Gestión/Unidad: </label>' .$planificacionAnual['gestion'].'
