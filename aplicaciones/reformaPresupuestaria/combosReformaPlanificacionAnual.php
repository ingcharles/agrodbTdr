...
$idAreaN4FiltroRevisionDGPGE = htmlspecialchars ($_POST['areaN4FiltroRevisionDGPGE'],ENT_NOQUOTES,'UTF-8');
...
$idAreaGestionRevisionDGPGE = htmlspecialchars ($_POST['gestionFiltroRevisionDGPGE'],ENT_NOQUOTES,'UTF-8');
...
switch ($opcion){
	...
	case 'responsable':
		$responsable = $cu->listarUsuariosXArea($conexion, $idAreaGestion);
	...
	//Revisión DGPGE
	case 'n4FiltroRevisionDGPGE':
	
		$areasN4 = $ca->buscarEstructuraPlantaCentralProvinciasXCategoria($conexion, '(4)');
	
		echo '<label id="lAreaN4">N4 - Dirección/Dirección Distrital:</label>
		<select id="areaN4FiltroRevisionDGPGE" name="areaN4FiltroRevisionDGPGE" required>
		<option value="">Seleccione....</option>';
		while($fila = pg_fetch_assoc($areasN4)){
			if($fila['id_area_padre'] == $idAreaN2){
				echo '<option value="' . $fila['id_area'] . '">' . $fila['nombre'].' </option>';
	...
	case 'gestionFiltroRevisionDGPGE':
	
		$areasGestion = $ca->buscarDivisionEstructura($conexion, $idAreaN4FiltroRevisionDGPGE);
	
		echo '<label id="lAreaGestion">Gestión/Unidad:</label>
		<select id="gestionFiltroRevisionDGPGE" name="gestionFiltroRevisionDGPGE" required>
		<option value="">Seleccione....</option>';
		while($fila = pg_fetch_assoc($areasGestion)){
			echo '<option value="' . $fila['id_area'] . '">' . $fila['nombre'].' </option>';
		}
	...
	case 'tipoFiltroRevisionDGPGE':
	
		echo '<label id="lTipo">Tipo:</label>
		<select id=tipoFiltroRevisionDGPGE name="tipoFiltroRevisionDGPGE" required="required">
		<option value="">Seleccione....</option>
		<option value="Proyecto Inversion">Proyecto Inversion</option>
		</select>';
	...
	//PAC
		case 'proyectoReportePAC':
			...
			echo '<select id="proyectoReporte" name="proyectoReporte" >
					<option value="">Seleccione....</option>
					<option value="">Todos</option>';
		...
		case 'actividadReportePAC':
			...
			echo '<select id="actividadReporte" name="actividadReporte" >
				<option value="">Seleccione....</option>
				<option value="">Todos</option>';
...
<script type="text/javascript">
	...
	$("#objetivoEspecifico").change(function(event){
		...
		$("#nuevaPlanificacionAnual").attr('data-opcion', 'combosReformaPlanificacionAnual');
	...
	$("#areaN4FiltroRevision").change(function(event){
		...
		$("#filtrarPlanificacionAnual").attr('data-opcion', 'combosReformaPlanificacionAnual');
	...
	//Revision DGPGE
	$("#areaN4FiltroRevisionDGPGE").change(function(event){
		$("#nombreAreaN4").val($("#areaN4FiltroRevisionDGPGE option:selected").text());

		$("#filtrarPlanificacionAnual").attr('data-opcion', 'combosReformaPlanificacionAnual');
	    $("#filtrarPlanificacionAnual").attr('data-destino', 'dGestion');
	    $("#opcion").val('gestionFiltroRevisionDGPGE');
	    
 	 	abrir($("#filtrarPlanificacionAnual"),event,false);
	 });

	$("#gestionFiltroRevisionDGPGE").change(function(event){
		$("#idGestion").val($("#gestionFiltroRevisionDGPGE option:selected").val());
		$("#nombreGestion").val($("#gestionFiltroRevisionDGPGE option:selected").text());

		$("#filtrarPlanificacionAnual").attr('data-opcion', 'combosReformaPlanificacionAnual');
	    $("#filtrarPlanificacionAnual").attr('data-destino', 'dTipo');
	    $("#opcion").val('tipoFiltroRevisionDGPGE');
	    
 	 	abrir($("#filtrarPlanificacionAnual"),event,false);
	 });
	 
	$("#tipoFiltroRevisionDGPGE").change(function(event){
		$("#filtrarPlanificacionAnual").attr('data-opcion', 'listaPlanificacionAnualFiltrada');
	    $("#filtrarPlanificacionAnual").attr('data-destino', 'contenedor');
	 });
