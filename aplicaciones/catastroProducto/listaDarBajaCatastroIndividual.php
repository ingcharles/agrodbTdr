...
switch ($filaTipoUsuario['codificacion_perfil']){
    ...
    case 'PFL_USUAR_EXT':
        ...
        if(stristr($codigoTipoOperacion, 'OPT') == true){
            ...
            $qResultadoEmpresaOperador = $cp->consultarEmpresaPorOperacion($conexion, "('OPI', 'PRO', 'COM')" , $identificadorUsuario);
...
$res = $cp->consultaCatastroIndividual($conexion, $_POST['identificadorSolicitanteH'],$_POST['nombreOperadorSolicitante'],$_POST['nombreSitio'],$_POST['provincia'],$_POST['fechaInicio'],$_POST['fechaFin'],$_SESSION['usuario'],$_POST['identificadorProductoUnico']);
...
while($fila = pg_fetch_assoc($res)){
	...
	$itemsFiltrados[] = array('<tr
		...
		data-opcion="abrirDarBajaCatastroIndividual"
...
<header>
		...
		<?php			
			...
			while($fila = pg_fetch_assoc($res)){
			    ...
			    if($banderaValidarAcciones){
			        $validacion = ($fila['estilo']!= "_eliminar");
			    }
	...
	<form id="nuevoFiltroCatastro" data-rutaAplicacion="catastroProducto" data-opcion="listaDarBajaCatastroIndividual" data-destino="areaTrabajo #listadoItems">
		...
		<table class="filtro" >
			<tbody>
				...
				<tr>
					...
					<td><input name="nombreOperadorSolicitante" id="nombreOperadorSolicitante"  type="text"  /></td>		
				</tr>
...
<script>	
	...
	$("#_eliminar").click(function(event){
		...
		if($("#cantidadItemsSeleccionados").text()==0){
			$("#mensajeError").html("Por favor seleccione un registro de catastro a dar de baja.").addClass('alerta');
			return false;
		}
