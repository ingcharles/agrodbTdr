...
<form id='formulario' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>Laboratorios'
    ...
    <fieldset>
        ...
        <div class="pestania" id="div_paso1">
            ...
            <fieldset>
                ...
                <div data-linea="6">
                    ...
                    <input type="number" id="cantidad" name="cantidad" min="1" size="5"/>
                </div>
                <div data-linea="6">
                </div>                

                <div data-linea="7">
                    <div id="div_provOrigenMuestra" class="cDatosGenerales">
                        <label>Provincia origen muestra</label> 
                        <select
                            id="provOrigenMuestra" name="provOrigenMuestra" >
                            <option value="">Seleccionar....</option>
                            <?php
            ...
            <fieldset>
                <legend class='legendMuestras'>Servicios guardados</legend>
                <div id="paginacionSolicitados" class="normal"></div>
                <table width="100%" id="tblDetalleSolicitudesGuardado">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Laboratorio</th>
                            <th>Tipo de an&aacute;lisis</th>
                            <th>N&uacute;mero de muestras</th>
                            <th>Costo U.</th>
                            <th>Subtotal</th>
                            <th>Editar</th>
                            <th>Borrar</th>
                            <th>Agregar Servicios</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                    <tfoot>
                        ...
                        <tr>
                            <th colspan="5" style="text-align: right">TOTAL SOLICITUD:</th>
                            <th id="totalSolicitud" style="text-align: right"></th>
                            <th colspan="2"></th>
                        </tr>
                    </tfoot>
                </table>
                <button type="button" id="btnImpProforma" class="fas fa-print"> Imprimir proforma</button>
            </fieldset>
        </div>
        ...
        <div data-linea="21">
            ...
            <input type="hidden" name="id_codigo_proforma" id="id_codigo_proforma"
...
<!-- Modal para datos de la proforma -->
...
<?php require APP . 'Laboratorios/vistas/modalConfirmacion.php'; ?>
<?php require APP . 'Laboratorios/vistas/modalConfirmacionAgregarAnalisis.php'; ?>
<?php require APP . 'Laboratorios/vistas/modalAgregarServicios.php'; ?>
...
<?php echo $this->codigoJS; ?>
    ...
    $(document).ready(function () {
        ...
        $("#tblDetalleSolicitudesGuardado tbody").html("<?php echo $this->detalleSolicitudesGuardado; ?>");
        fn_sumarCosto();
    ...
    //eliminar detalle_solicitudes guardado
    function fn_eliDetSol(idSolicitud, idDetalleSolicitud) {
        respuesta = confirm("Desea eliminar?");
        if (respuesta) {
            $.post("<?php echo URL ?>Laboratorios/Solicitudes/borrarDetalleSolicitudes/" + idSolicitud + "/" + idDetalleSolicitud,
                    function (data) {
    ...
    //Para seleccionar un solo registro y obtener los ids de servicio que corresponde
    //al laboratorio seleccionado
    function fn_checkEditar(selected, id_laboratorio) {
        $("#id_detalle_solicitud").val($(selected).val());
        ...
        if ($(".chk_detalle").is(':checked')) {
            $.post("<?php echo URL ?>Laboratorios/Solicitudes/getTotalMuestras/" + $("#id_solicitud").val() + "/" + $("#id_laboratorio").val(),
                    function (data) {
                        $("#list_id_servicio").val(data.servicios);
                        $("#list_cantidad").val(data.cantidades);
                    }, "json");
        } else {
            $("#list_id_servicio").val("");
            $("#list_cantidad").val("");
        }
        fn_obtenerDatosLaboratorio();
    }

    //Funcion par sumar los costos
    function fn_sumarCosto() {
        ...
        var subTotalG = 0;
        $(".list_costo_guardado").each(function () {
            subTotalG = parseFloat(subTotalG) + parseFloat($(this).text());
        });
        $("#subtotal").html("$ " + subTotalG.toFixed(2));
                var ivaG = subTotalG * 0.<?php echo \Agrodb\Core\Constantes::IVA; ?>;
                $("#iva").html("$ " + ivaG.toFixed(2));
                var total = subTotalG * 1.<?php echo \Agrodb\Core\Constantes::IVA; ?>;
                $("#total").html("$ " + total.toFixed(2));
        //total de la solicitud: servicios que ingresa + servicios guardados
        $("#totalSolicitud").html("$ " + (totalS + total).toFixed(2));
    }
            ...
            if (fn_validarSolicitud(2, pestaniaActual) === 1) { //validar paso 2
                if ($("#id_laboratorio").val() !== null) {
                    ...
                    if ($('#serMArbetes').val() === 'true') {
                        ...
                        $.post("<?php echo URL ?>Laboratorios/Solicitudes/camposAnalisis/" + $("#id_laboratorio").val() + "/<?php echo $this->modeloSolicitudes->getTipoSolicitud(); ?>",
                                {
    ...
    // IMPRIMIR PROFORMA
    //La proforma se debe realizar una vez registrada la información de las muestras y el tipo de análisis
    $("#btnImpProforma").click(function () {
        if ($("#id_laboratorio").val() !== "") {
            $.post("<?php echo URL ?>Laboratorios/Proformas/vistaProforma/",
                    function (data) {
                        $('#modalProforma').modal('show');
                        $("#divProforma").html(data);
                    });
        } else {
            mostrarMensaje("Debe ingresar datos en la solicitud.", "FALLO");
        }
    });
</script>
