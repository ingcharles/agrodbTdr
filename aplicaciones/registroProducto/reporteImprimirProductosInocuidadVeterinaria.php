...
$ext   = '.xls';
$nomReporte = "REPORTE RPIV".$fecha.$ext;
...
$res = $rpip-> reporteImprimirProductosInocuidadVeterinaria($conexion);
...
<body>
	<div id="header">
		...
		<div id="textoPOA" style="font-size: 16px; font-weight: bold;">
			...
			AGROCALIDAD<br>REPORTE DE PRODUCTOS DE INSUMOS VETERINARIOS<br>
		</div>
	...
	echo '<tr>';
		...
		echo'<td>';
			foreach ((array)$registros['usos'] as $usos) { 
				$usoss.=$usos['nombre_uso'].' Aplicado a '. $usos['nombre_especie'].', ';
			}
			echo rtrim ($usoss, ' ,' );
		echo '</td>';
		...
		echo '<td>'.$registros['nombre_empleado'].'</td>';
