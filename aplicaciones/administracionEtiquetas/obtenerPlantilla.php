...
try{
	...
	try {
		...
		while($plantillaImagen=pg_fetch_assoc($res)){
		    $option.= '<option value="'.$plantillaImagen['codigo'].'" data-ruta="'.$plantillaImagen['ruta'].'" >'.$plantillaImagen['nombre'].'</option>';
		}	
		...
		$contenido= '
			...
			<div data-linea="5">
				<label for="cbPlantilla">Plantilla:</label>
					<select id="cbPlantilla" name="cbPlantilla" disabled>'.$option.'
				</select>
			</div>
			<div data-linea="7" >
		...
		<fieldset>
			...
			<div data-linea="1" id="resultadoPlantilla">
				<div style="text-align:center;width:100%" id="plantillaImagen">				
				</div>
			</div>
		</fieldset>
		<fieldset>
