		...
		try {
			...
			switch ($plantillaProducto['plantilla']) {
				case 'P1':
					...
					$imagenPlantilla = pg_fetch_assoc($cl->obtenerPlantillasRutaXcodigoYtipo($conexion, 'P1', 2));
					...
					if ($plantillaProducto['hoja'] == 'etiqueta') {
						...
						for ($i = 0; $i <= $nroEtiqueta - 1; $i++) {
							$pdf->AddPage();
							$pdf->Image($imagenPlantilla['ruta'], $pdf->SetY($fila), $pdf->SetX(0), 100, 50, 'PNG');
							$pdf->Image($rutaImagenQr, $pdf->SetY(10.5), $pdf->SetX(0.7), 29, 29, 'PNG');
							...
							$pdf->MultiCell(100, 2, utf8_decode($variedad) . substr(utf8_decode($tipo), 0, 39), 0, 'L', 0);
							...
							while ($contador < $nroEtiqueta) {
								...
								for ($i = 0; $i <= $plantillaProducto['cantidad'] - 1; $i++) {
									if ($contador < $nroEtiqueta) {
										...
										$pdf->Image($imagenPlantilla['ruta'], $pdf->SetY($fila), $pdf->SetX(12), 100, 50, 'PNG');
									...
									for ($j = 0; $j < 2; $j++) {
										...
										if ($contador < $nroEtiqueta) {
											...
											if ($limite <= $plantillaProducto['cantidad']) {
												...
												$pdf->SetFillColor(135, 229, 159);
												$pdf->Image($imagenPlantilla['ruta'], $pdf->SetY($fila), $pdf->SetX($posiIm), 100, 50, 'PNG');
				...
				case 'P2':
					...
					$imagenPlantilla = pg_fetch_assoc($cl->obtenerPlantillasRutaXcodigoYtipo($conexion, 'P2', 2));
				...
				case 'P3':
					...
					$imagenPlantilla = pg_fetch_assoc($cl->obtenerPlantillasRutaXcodigoYtipo($conexion, 'P3', 2));
					...
					$infoQr = 'Lote Envío:' . $nrLote . "\n" .
						'Centro de Tratamiento:' . $datosOperador . "\n" .
						'Sitio de Producción: ' . $datosProveedor . "\n" .
						'Producto:' . $producto . "\n" .
						'Lote Producción:' . $codigoLote . "\n" .
						'Fecha de Empaque:' . $fechaEtiquetado . "\n" .
						'Destino:' . $pais . "\n" .
				...
				case 'P4':
					...
					$datosOperador = $operador . '.' . $areas['area_operador'];
					//$datosProveedor= $proveedores['identificador_proveedor'].'.'.$areas['area_proveedor'].'-'.$proveedores['nombre_proveedor'];
					$nombreProveedor = explode('-', $proveedores['nombre_proveedor']);

					$datosProveedor = $nombreProveedor[0];

					$imagenPlantilla = pg_fetch_assoc($cl->obtenerPlantillasRutaXcodigoYtipo($conexion, 'P4', 2));
					...
					$infoQr = 'Lote Envío:' . $nrLote . "\n" .
						'Cod. Lugar de Producción:' . $datosOperador . "\n" .
						'Cod. Centro de acopio: ' . $datosProveedor . "\n" .
						'Producto:' . $producto . "\n" .
						'Lote Producción:' . $codigoLote . "\n" .
						'Fecha de Empaque:' . $fechaEtiquetado . "\n" .
						'Destino:' . $pais . "\n" .
					...
					if ($plantillaProducto['hoja'] == 'A4') {
							...
							while ($contador < $nroEtiqueta) {
								...
								for ($i = 0; $i <= $plantillaProducto['cantidad'] - 1; $i++) {
									if ($contador < $nroEtiqueta) {
										...
										$fila += 20;
										$pdf->SetXY(56, $fila);
										$pdf->MultiCell(80, 3, $producto, 0, 'L', 0);

										$pdf->SetXY(89, $fila);
										$pdf->MultiCell(70, 3, substr(utf8_decode($codigoLote), 0, 100), 0, 'L', 0);

										$fila += 5;
										$pdf->SetXY(54, $fila);
										$pdf->MultiCell(70, 2.8, substr(utf8_decode($pais), 0, 100), 0, 'L', 0);

										$fila += 6;
										$pdf->SetXY(72, $fila);
										$pdf->MultiCell(70, 2, $datosOperador, 0, 'L', 0);

										$fila += 6;
										$pdf->SetXY(67, $fila);
										$pdf->MultiCell(70, 2, $datosProveedor, 0, 'L', 0);

										$fila += 6;
										$pdf->SetXY(69, $fila);
										$pdf->MultiCell(55, 2, substr(utf8_decode($fechaEtiquetado), 0, 70), 0, 'L', 0);
									...
									for ($j = 0; $j < 2; $j++) {
										...
										if ($contador < $nroEtiqueta) {
											...
											if ($limite <= $plantillaProducto['cantidad']) {
												...
												$pdf->SetFont('Arial', '', 7);
												$fila += 20;
												$pdf->SetXY($posiX + 14, $fila);
												$pdf->MultiCell(80, 3, $producto, 0, 'L', 0);

												$pdf->SetXY($posiX + 47, $fila);
												$pdf->MultiCell(70, 3, substr(utf8_decode($codigoLote), 0, 100), 0, 'L', 0);

												$fila += 5;
												$pdf->SetXY($posiX + 12, $fila);
												$pdf->MultiCell(70, 2.8, substr(utf8_decode($pais), 0, 100), 0, 'L', 0);

												$fila += 6;
												$pdf->SetXY($posiX + 31, $fila);
												$pdf->MultiCell(70, 2, $datosOperador, 0, 'L', 0);

												$fila += 6;
												$pdf->SetXY($posiX + 26, $fila);
												$pdf->MultiCell(100, 2, $datosProveedor, 0, 'L', 0);

												$fila += 6;
												$pdf->SetXY($posiX + 27, $fila);
												$pdf->MultiCell(55, 2, substr(utf8_decode($fechaEtiquetado), 0, 70), 0, 'L', 0);

												$fila += 4;
												$pdf->SetXY($posiX + 48, $fila);
												$pdf->MultiCell(50, 2, $nCajas . '-' . $nroEtiqueta, 0, 'L', 0);
						...
						for ($i = 0; $i <= $nroEtiqueta - 1; $i++) {
							...
							$pdf->SetXY(77, $fila);
							$pdf->MultiCell(45, 2, utf8_decode($codigoLote), 0, 'L', 0);
							...
							$fila += 7;
							$pdf->SetXY(61, $fila);
							$pdf->MultiCell(72, 1, substr(utf8_decode($datosOperador), 0, 100), 0, 'L', 0);
