...
header("Content-Disposition: attachment; filename=REPORTE_PROTOCOLO_DESVITALIZACION.xls");
...
$res = $cr->generarReporteProtocoloDesvitalizacion($conexion, $fechaInicio, $fechaFin);
?>
...
<h1>Reporte de Protocolo de desvitalización</h1>
<h2>Período <?= $fechaInicio ?> - <?= $fechaFin ?></h2>
<div id="tabla">
    <table id="tablaReporte" class="soloImpresion">
        <thead>
        <?php
        echo $cr->construirEncabezadoReporte($conexion, array('certificacionf04', 'certificacionf04_detalle_productos'), $campos);
        ?>
        </thead>
        <tbody>
        <?php
