...
<div class="pestania">
	<form id='formularioFormaFisFarCosProducto' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>DossierPecuario' data-opcion='FormaFisFarCosProducto/guardar' data-destino="detalleItem" method="post" class="editable">
    	...
    	<fieldset>
    		<legend>Forma Física</legend>				
    
    		<div data-linea="1">
    			<label for="id_forma">Forma física: </label>
            	...
            	<div data-linea="3">
        			<table id="tbItemsFormaFisFarCosProducto" style="width:100%">
        				<thead>
        					<tr>
        						<th style="width: 10%;">#</th>
        						<th style="width: 80%;">Forma física</th>
                                <th style="width: 10%;"></th>
        					</tr>
        				</thead>
        				<tbody>
    ...
    <form id='formularioPaso5' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>DossierPecuario' data-opcion='solicitud/guardar' data-destino="detalleItem" method="post">
    	...
    	<fieldset>
    		...
    		<div data-linea="6">
    			<label for="metodo_bromatologico">Método bromatológico: </label>
    			<input type="text" id="metodo_bromatologico" name="metodo_bromatologico" value="<?php echo $this->modeloSolicitud->getMetodoBromatologico(); ?>" maxlength="1024" class="formulario" />
    		</div>	    		
    		    		
    		<div data-linea="7">
    			<label for="pruebas_microbiologicas">Pruebas microbiológicas: </label>
    			<input type="text" id="pruebas_microbiologicas" name="pruebas_microbiologicas" value="<?php echo $this->modeloSolicitud->getPruebasMicrobiologicas(); ?>" maxlength="1024" class="formulario" />
    		</div>
		
			<div data-linea="8">
    			<label for="metodo_fis_quim">Método físico-químico: </label>
    			<input type="text" id="metodo_fis_quim" name="metodo_fis_quim" value="<?php echo $this->modeloSolicitud->getMetodoFisQuim(); ?>" maxlength="1024" class="formulario" />
    		</div>				
    	</fieldset>	
...
<div class="pestania">
    ...
    <form id='formularioFormaAdministracion' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>DossierPecuario' data-opcion='FormaAdministracion/guardar' data-destino="detalleItem" method="post" class="editable">
    	...
    	<fieldset>
    		<legend>Forma de Administración en Animales</legend>				
   ...
   <form id='formularioFormaAdministracionAlimento' data-rutaAplicacion='<?php echo URL_MVC_FOLDER; ?>DossierPecuario' data-opcion='FormaAdministracionAlimento/guardar' data-destino="detalleItem" method="post" class="editable">
    	<input type="hidden" id="id_solicitud" name="id_solicitud" value="<?php echo $this->modeloSolicitud->getIdSolicitud(); ?>" />
    	
    	<fieldset>
    		<legend>Forma de Administración en el Alimento</legend>				
    
    		<div data-linea="1">
    			<label for="dosis_alimento">Dosis en alimentos: </label>
    			<input type="text" id="dosis_alimento" name="dosis_alimento" maxlength="256" />
    		</div>				
    
    		<div data-linea="2">
    			<label for="forma_administracion">Forma de administración: </label>
    			<input type="text" id="forma_administracion" name="forma_administracion" maxlength="256" />
    		</div>
 	...
 	<div id="tablaFormaAdministracionAlimento">
        <fieldset>
        	<legend>Forma de Administración en el alimento</legend>
            	<div data-linea="3">
        			<table id="tbItemsFormaAdministracionAlimento" style="width:100%">
        				<thead>
        					<tr>
        						<th style="width: 5%;">#</th>
        						<th style="width: 45%;">Dosis en alimentos</th>
        						<th style="width: 45%;">Forma de administración</th>
                                <th style="width: 5%;"></th>
        					</tr>
        				</thead>
        				<tbody>
...
var combo = "<option>Seleccione....</option>";
	...
	$(document).ready(function() {
		...
		fn_mostrarDetalleFormaAdministracionAlimento();
  ...
  //Para origen de producto grid
    ...
    $("#formularioPaso5").submit(function (event) {
		...
		if($.trim($("#caracteristicas_fis_quim_org").val())){
			if(!esCampoValido("#caracteristicas_fis_quim_org")){
            	error = true;
            	$("#caracteristicas_fis_quim_org").addClass("alertaCombo");
			}
		}

		if($.trim($("#metodo_bromatologico").val())){
			if(!esCampoValido("#metodo_bromatologico")){
            	error = true;
            	$("#metodo_bromatologico").addClass("alertaCombo");
			}
		}

		if($.trim($("#pruebas_microbiologicas").val())){
			if(!esCampoValido("#pruebas_microbiologicas")){
            	error = true;
            	$("#pruebas_microbiologicas").addClass("alertaCombo");
			}
		}
    ...
    $("#formularioFormaAdministracionAlimento").submit(function (event) {
		fn_limpiar();
		event.preventDefault();
		var error = false;
		
		if(!$.trim($("#dosis_alimento").val())){
        	error = true;
        	$("#dosis_alimento").addClass("alertaCombo");
		}
		
		if(!$.trim($("#forma_administracion").val())){
        	error = true;
        	$("#forma_administracion").addClass("alertaCombo");
		}
		...
		if (!error) {
	        var respuesta = JSON.parse(ejecutarJson($(this)).responseText);
	       	if (respuesta.estado == 'exito'){
	       		fn_mostrarDetalleFormaAdministracionAlimento();
	       		fn_limpiarFormaAdministracionAlimentoCompleto();
	        }else{
	        	fn_mostrarDetalleFormaAdministracionAlimento();
	        }
		} else {
    ...
    //Para cargar el detalle de Forma de Administración en Alimento
    function fn_mostrarDetalleFormaAdministracionAlimento() {
        var idSolicitud = $("#id_solicitud").val();
        
    	$.post("<?php echo URL ?>DossierPecuario/FormaAdministracionAlimento/construirDetalleFormaAdministracionAlimento/",
    	{
			idSolicitud : idSolicitud,
			fase : bandera
		}, function (data) {
            $("#tbItemsFormaAdministracionAlimento tbody").html(data);
        });
    }

  	//Funcion que elimina una fila de la lista 
    function fn_eliminarDetalleFormaAdministracionAlimento(idDetalleFormaAdministracionAlimento) { 
        $.post("<?php echo URL ?>DossierPecuario/FormaAdministracionAlimento/borrar",
        {                
            elementos: idDetalleFormaAdministracionAlimento
        },
        function (data) {
        	fn_mostrarDetalleFormaAdministracionAlimento();
        });
    ...
    function fn_limpiarFormaAdministracionAlimentoCompleto() {
    	$("#dosis_alimento").val("");
    	$("#forma_administracion").val("");
    }
    ...
    $("#formularioDocumentoAnexo").submit(function (event) {
		...
		/*if(!$.trim($("#descripcion_documento_anexo").val())){
        	error = true;
        	$("#descripcion_documento_anexo").addClass("alertaCombo");
		}*/
