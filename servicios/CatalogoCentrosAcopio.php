 ...
 * Time: 17:38
 *
 * Catálogo para formulario CertificaciónF02(Lugares de rechazo) y F10
 * Lista sitios y centros de acopio en una provincia
 *
 */
class CatalogoCentrosAcopio extends Servicio
{
        ...
        if ($this->provincia != null || $this->provincia != '') {
            $consulta = 'SELECT row_to_json(res) FROM ( SELECT array_to_json(array_agg(row_to_json(listado))) FROM (' .
                "SELECT 
                 ...
                 s.canton,
                 s.provincia,
                 s.latitud as \"coordenadaX\",
                 s.longitud as \"coordenadaY\",
