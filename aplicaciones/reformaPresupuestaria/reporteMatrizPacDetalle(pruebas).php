...
/*if($_POST['areaDireccion'] == 'Todos'){
	...
	if($nombreArea == 'Fortalecimiento Institucional'){
		$completo = $crp->obtenerReportePacFortalecimiento($conexion, $_POST['areaN2'], $_POST['codigoProgramaPAC'], $_POST['codigoProyectoPAC'], 
													$_POST['codigoActividadPAC'], $_POST['provincia'], $anio, $estadoReporte);
	}else{
		$completo = $crp->obtenerReportePac($conexion, $_POST['areaN2'], $_POST['codigoProgramaPAC'], $_POST['codigoProyectoPAC'],
				$_POST['codigoActividadPAC'], $_POST['provincia'], $anio, $estadoReporte);
	}
...
<div id="header">
	...
	<div id="imprimir">
	<form id="filtrar" action="reporteMatrizPacDetalleExcel.php" target="_blank" method="post">
...
<table id="tablaReportePresupuesto" class="soloImpresion">
	<thead>
		<tr>
			...
			<th>Revisor GF</th>
			<th>Fecha Revisión GF</th>
			<th>Observaciones Revisión GF</th>
	...
	<tbody>
	 ...
	 while($fila = pg_fetch_assoc($completo)){
	 	...
	 	$revidorGF = pg_fetch_assoc($cu->obtenerNombresUsuario($conexion, $fila['identificador_revisor_gf']));
	 	$revisorPresupuestoGF = pg_fetch_assoc($cu->obtenerNombresUsuario($conexion, $fila['identificador_revisor_gf']));
		...
		<td class="formatoNumeroDecimal4">'.abs($fila['costo'] - $fila['costo_original']).'</td>
		...
		<td>'.$revisorPresupuestoGF['nombre'].' '.$revisorPresupuestoGF['apellido'].'</td>
		<td>'.$fila['fecha_revision_gf'].'</td>
		<td>'.$fila['observaciones_revision_gf'].'</td>
