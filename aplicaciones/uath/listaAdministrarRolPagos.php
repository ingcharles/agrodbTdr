...
$contador = 0;
if(isset($_POST['areaR'])){
$res = $cc->buscarExcelRolPagos($conexion, $_POST['mesH'], $_POST['anoH'], $_POST['areaR']);
	

while($fila = pg_fetch_assoc($res)){
$contenido ='<article
	    		id="'.$fila['id_excel_rol'].'"
				class="item"
				data-rutaAplicacion="uath"
				data-opcion="listaAdministrarRolPagosIndividual" 
	    		ondragstart="drag(event)"
				draggable="true"
				data-destino="aprobado">
				<span class="ordinal">'.++$contador.'</span>
				<span>'.$fila['mes_rol'].'</span>
				<aside><strong>Año: </strong>'	.$fila['anio'].'</aside>
			</article>';
	?>
...
<header>
	...
	<form id="nuevoFiltroRolPagos" data-rutaAplicacion="uath" data-opcion="listaAdministrarRolPagos" data-destino="areaTrabajo #listadoItems">
		<input type="hidden" name="opcion" value="<?php echo $_POST['opcion']; ?>" />
		
		<table class="filtro" style='width: 325px;'>
			<tbody>
				<tr>
					<th colspan="4">Consultar Roles:</th>
				</tr>
				<tr>
					<td>* Área:</td>
					<td><select style='width:100%' name="areaR" id="areaR" >
...
<script>	
	...
	$("#nuevoFiltroRolPagos").submit(function(event){
