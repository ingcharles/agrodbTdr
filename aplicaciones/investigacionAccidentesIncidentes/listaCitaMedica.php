...
<header>
		<h1>Accidente</h1>
			<nav>
				<form id="filtrar" data-rutaAplicacion="investigacionAccidentesIncidentes" data-opcion="listaCitaMedica" data-destino="areaTrabajo #listadoItems" >
				...
				<?php if($identificador != ''){ 
					...
					filtroBusqueda($_POST['identificador'],$_POST['solicitud'],$_POST['fechaBusque'],2);
				 }?>
				</form>
			</nav>
			</header>
		...
		<?php 
				...
				if($idArea['idarea'] == 'DGATH' )
					$consulta=$cai->listarDatosAccidente($conexion,'', $estadoSolicitud,$id_area_padre=NULL,
							$tipo_Sso=NULL, 2,$solicitud, $identificadorBusque,$fechaBusque);
				else $consulta=$cai->listarDatosAccidente($conexion,'', '',$areaPadre,
						$tipo_Sso=NULL, 2,$solicitud, $identificadorBusque,$fechaBusque);
				...
				$contador = 0;
					...
					while($cita = pg_fetch_assoc($consulta)){	
						...
						$ruta='nuevaCitaMedica';
						if($cita['estado']=='Subsanar')$ruta='modificarCitaMedica';
