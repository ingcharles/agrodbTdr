	...
	try {
		...
		if(pg_num_rows($cmp->consultarRolEmpleado($conexion, $datos['rol'], $datos['idEmpleado'])) == 0){
		    ...
		    $identificadorEmpresa = pg_fetch_result($cee->obtenerIdentificadorEmpresaEmpleado($conexion, $datos['idEmpleado']), 0, 'identificador_empresa');
		        ...
		        $qOperacionesEmpresa = $cee->obtenerOperadorEmpresa($conexion, $identificadorEmpresa,"('FAEAI')");
    			...
    			if (pg_num_rows($qOperacionesEmpresa) > 0){
    			    ...
    			    while($filaAplicacion = pg_fetch_assoc($qGrupoAplicacion)){
    			        ...
    			        $qGrupoPerfiles = $cgap->obtenerGrupoPerfilXAplicacion($conexion, $filaAplicacion['id_aplicacion'], "('PFL_EMISO_MOVIL', 'PFL_FISCA_MOVIL')");
    			        $perfilesArray = Array();
    			    ...
    			    $mensaje ['mensaje'] = 'Usted no cuenta con operaciones de centros de faenamientos para agregar digitadores de fiscalización';
    			}
    	}else{
			$mensaje ['estado'] = 'error';
