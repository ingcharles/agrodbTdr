...
<body>
<header>
		<h1>Aprobación de informes finales</h1>
		<nav>
		<?php 
	...
	<div id="aprobarProtocoloDir">
		<h2>Por aprobar protocolo Director</h2>
		<div class="elementos"></div>
	</div>
	...
	$estado='aprobarInformeDir';
	$flujo=$ce->obtenerFlujo($conexion,$_SESSION['idAplicacion'],$estado);
	$yaDesplego=false;
	$tramites =array();
	$perfiles=array();
	$perfilActual='PFL_EE_DRIA';
	if($perfil->esDirector() ){

		
		$tramitesRecuperados=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$flujo['id_fase'],$estado,$perfilActual,'N');
		while($fila = pg_fetch_assoc($tramitesRecuperados)){
			$tramites[]=$fila;
		}
		$perfiles[]=$perfilActual;
	}
	else if( $perfil->EsDirectorTipoA()){
	...
	else if($perfil->EsCoordinador()){
		$perfilActual='PFL_EE_CRIA';
		$estadoBuscado='aprobarInformeCor';
		$fase_flujo=$ce->obtenerFaseDelFlujo($conexion,$flujo['id_flujo'],$estadoBuscado);

		$tramitesRecuperados=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$fase_flujo,$estadoBuscado,$perfilActual,'N');
		while($fila = pg_fetch_assoc($tramitesRecuperados)){
			$tramites[]=$fila;
		}
		$perfiles[]=$perfilActual;
	}
	else if($perfil->EsOperador()){

		$tramites=$ce->obtenerTramitesPendientesDelOperadorEE($conexion,$flujo['id_fase'],$identificador,'N');
	}
	...
	if(sizeof($tramites)>0){
		...
		foreach($tramites as $fila)
		{
			...
			$contenido = '<article
							...
							data-opcion="abrirAprobacionInforme"
