 ...
 * Time: 17:38
 *
 * Catálogo para formulario Certificación F07 F05 F08 F09
 * Lista sitios y centros de acopio en una provincia
 *
 */
class CatalogoSitioProduccionFlores extends Servicio
{
            ...
            $consulta = 'SELECT row_to_json(res) FROM ( SELECT array_to_json(array_agg(row_to_json(listado))) FROM (' .
                "SELECT
                 ...
                 WHERE
                     ...
                     tp.id_area = 'SV' and  
                     tp.codigo IN ('PRO','ACO','COM') and
                     a.estado = 'creado' and
                     op.estado not in ('noHabilitado') and
                     upper(s.provincia) = upper('" . $this->provincia . "') and
                     tpr.codificacion_tipo_producto = 'PRD_FLO_FOLL_COR'
                 ORDER BY
                     a.nombre_area"
                    . ') AS listado ) AS res;';
