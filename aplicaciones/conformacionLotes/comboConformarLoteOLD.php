...
switch ($opcion){
	...
	case 'registros':
	    ...
	    // $PivoColumna = $cl->pivotearColumnas($conexion, $producto, $formulario['id_formulario']);
	        ...
	        $registrosTotal=pg_fetch_assoc($ca->obtenerRegistrosTabla($conexion, 'v_caracteristica',$productoID));
		...
		echo '<label>Seleccione uno o varios Productos</label>		
			 ...
			 <div id="contenedorProducto">
        		...
        		$registrosTotal=pg_fetch_assoc($ca->obtenerRegistrosTabla($conexion, 'v_caracteristicaTotal', $productoID));
