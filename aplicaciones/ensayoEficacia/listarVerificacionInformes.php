...
<body>
<header>
		<h1>Verificación de informes finales</h1>
		<nav>
		<?php 
	...
	<div id="verificacionInforme">
		<h2>Informes finales por revisar</h2>
		<div class="elementos"></div>
	</div>
	...
	<?php
		...
		$estadoSolicitud="verificacionInforme";
		$flujo=$ce->obtenerFlujo($conexion,$_SESSION['idAplicacion'],$estadoSolicitud);

		$tramites =array();
		$paginaSiguiente='abrirVerificarInforme';

		if($perfil->EsAnalistaCentral()){

			$tramitesRecuperados=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$flujo['id_fase'],$estadoSolicitud,'PFL_EE_ARIA','N');
			while($fila = pg_fetch_assoc($tramitesRecuperados)){
				$tramites[]=$fila;
			}
			$paginaSiguiente='abrirVerificarInforme';
		}
		if($perfil->EsAnalistaDistrital()){

			$tramitesRecuperados=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$flujo['id_fase'],$estadoSolicitud,'PFL_EE_ADTA','N');
			while($fila = pg_fetch_assoc($tramitesRecuperados)){
				$tramites[]=$fila;
			}
			$paginaSiguiente='abrirVerificarInforme';
		...
		foreach($tramites as $key=>$fila)
		{
			...
			$contenido = '<article
                            ...
                            <span>'.(strlen($fila['id_expediente'])>45?(substr($fila['id_expediente'],0,45).'...'):(strlen($fila['id_expediente'])>0?$fila['id_expediente']:'X definir')).'</span><br/>
