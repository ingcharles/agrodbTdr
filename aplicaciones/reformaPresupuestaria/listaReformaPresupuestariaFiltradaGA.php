...
<body>
	...
	<?php  
		...
		$res = $crp->listarProgramacionAnualTemporalVistaAprobacion($conexion, $areaN2, $areaN4, $idGestion, $tipo,
				$anio, $identificadorRevisor, "'enviadoRevisorGA','revisadoGA'", 'GA');
		...
		while($fila = pg_fetch_assoc($res)){
			...
			if($presupuestosRechazados>0){
				...
				}else if(($fila['tipo']=='Proceso') && ($fila['estado']=='revisadoGA')){
				...
				}else if(($fila['tipo']=='Proyecto Gasto Corriente') && ($fila['estado']=='revisadoGA')){
					if($presupuestosEnviadosRevisor>0){
				...
				}else if(($fila['tipo']=='Proyecto Inversion') && ($fila['estado']=='revisadoGA')){
					if($presupuestosEnviadosRevisor>0){
			...
			$contenido ='<article 
								...
								data-opcion="abrirReformaPlanificacionAnualRevisionGA" 
