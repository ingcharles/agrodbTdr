...
<header>
	<h1>Reporte matriz poa</h1>
	<nav>
		<!--form id="filtrar" action="aplicaciones/poa/reporteMPOAFiltrados.php" target="_blank" method="post"-->
		<form id="filtrar" action="aplicaciones/poa/reporteMPOAConsolidadoFiltrados.php" target="_blank" method="post">
			...
			<table class="filtro">
				...
				<tr>
					<th></th>
					<td>Cobertura:</td>
					<td><select id="listaCobertura" name="listaCobertura">
							<option value="">Todos</option>
					...
					<td>Población objetivo:</td>
					<td><select id="listaPoblacion" name="listaPoblacion">
							<option value="">Todos</option>
				<?php 
					$res= $cpoa1->listarPoblacionObjetivo($conexion, $fecha['year']);
    				while($fila = pg_fetch_assoc($res)){
				...
				<tr>
					<th></th>
					<td>Responsable:</td>
					<td><select id="ListaResponsable" name="ListaResponsable">
							<option value="">Todos</option>
				<?php 
					$res= $cpoa1->listarResponsable($conexion, $fecha['year']);
    				while($fila = pg_fetch_assoc($res)){
	                       echo '<option value="' . $fila['responsable']. '">' . $fila['responsable'] .'</option>';
					}?>
			</select></td>
					<td>Medios de Verificación:</td>
					<td><select id="listaVerificacion" name="listaVerificacion">
							<option value="">Todos</option>
				<?php 
					$res= $cpoa1->listarMediosVerificacion($conexion, $fecha['year']);
    				while($fila = pg_fetch_assoc($res)){
	                       echo '<option value="' . $fila['medios']. '">' . $fila['medios'] .'</option>';
					}?>
			</select></td>
				</tr>
				<!--tr>
					<th></th>
					<td>Indicador de los proyectos</td>
					<td><select name="codigo_Indicador" id="codigo_Indicador">
								<option value="">Todos</option>
							<?php 
					$res= $cpoa1->listarIndicadores($conexion, $fecha['year']);
    				while($fila = pg_fetch_assoc($res)){
	                       echo '<option value="' . $fila['id_indicador']. '">' . $fila['descripcion'] .'</option>';
					}?>
					</select>
					</td>
					<td colspan="5"></td>
