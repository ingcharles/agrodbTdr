 ...
 * Time: 17:38
 *
 * Catálogo para formulario Certificación02
 * Lista empresas de tratamiento de pallets una provincia
 *
 */
class CatalogoEmpresasTratamiento extends Servicio
{
    public function ejecutarServicio($registro)
    {
        if ($this->provincia != null || $this->provincia != '') {
            $consulta = 'SELECT row_to_json(res) FROM ( SELECT array_to_json(array_agg(row_to_json(listado))) FROM (' .
                "SELECT 
                 DISTINCT o.identificador \"rucCi\",
