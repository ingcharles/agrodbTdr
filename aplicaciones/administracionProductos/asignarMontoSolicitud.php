	...
	switch ($tipoSolicitud){
		case 'Operadores':
			...
			foreach($idSolicitud as $solicitud){
				$qOperacion=$cr->abrirOperacionXid($conexion, $solicitud);
			...
			$idOperadorTipoOperacion = rtrim($idOperadorTipoOperacion,',');echo '<br>';
			$idHistoricoOperacion = rtrim($idHistoricoOperacion,',');
			
		break;
		case 'Importación':
			$estado = 'verificacionVUE';
			$idOperadorTipoOperacion = 0;
			$idHistoricoOperacion = 0;
		break;
		case 'Fitosanitario':
			$estado = 'verificacionVUE';
			$idOperadorTipoOperacion = 0;
...
<form id="asignarMonto" data-rutaAplicacion="revisionFormularios" data-opcion="asignarMontoSolicitud" data-accionEnExito="ACTUALIZAR">
	...
	<input type="hidden" name="idSolicitud" value="<?php echo $datos[0];?>"/>
	...
	<input type="hidden" name="envioVue" value="<?php echo $tipoTarifario;?>"/>
	...
	<input type="hidden" id="idGupoSolicitudes" name="idGupoSolicitudes" value="<?php echo $idGrupo;?>">
			...
			<button type="button" onclick="agregarItem()" class="mas">Agregar Item</button>
				...
				<table id="tablaDetalle">
					...
					if($estadoActual != 'pago'){
						...
						$ordenPago = pg_fetch_assoc($cc->obtenerIdOrdenPagoXtipoOperacion($conexion, $idGrupo, $idSolicitud, $tipoSolicitud));
