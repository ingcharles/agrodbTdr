...
<body>
<header>
		<h1>Asignación de técnico para dossier</h1>
		<nav>
		<?php 
	...
	$perfilResponsable='PFL_RES_DISTRITO';
	if($perfil->tieneEstePerfil($perfilResponsable)){
		...
		if($division=='DIV_PICH'){
			...
			$ts=$cp->obtenerFlujosDeTramitesAsignarDossierDP($conexion,$identificador,$fase,$estado,$perfilResponsable,'N','not',"'RIA-FAR','RIP-BIO','RIP-KD'");
		}
		else
			$ts=$cp->obtenerFlujosDeTramitesAsignarDossierDP($conexion,$identificador,$fase,$estado,$perfilResponsable,'N');*/
		$ts=$cp->obtenerFlujosDeTramitesAsignarDossierDP($conexion,$identificador,$fase,$estado,$perfilResponsable,'N');
		/*FIN EJAR*/
	...
	if($perfil->tieneEstePerfil($perfilResponsable)){
		...
		if($division=='DIV_PICH'){
			...
			$ts=$cp->obtenerFlujosDeTramitesAsignarDossierDP($conexion,$identificador,$fase,$estado,$perfilResponsable,'N','',"'RIP-FAR','RIP-BIO','RIP-KD'");
		}
		while($fila = pg_fetch_assoc($ts))
		{
			$tramites[]=$fila;
