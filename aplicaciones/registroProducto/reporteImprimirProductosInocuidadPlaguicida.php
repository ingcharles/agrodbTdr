	...
	echo '<tr>';
		...
		echo '<td>';
    		foreach ((array)$registros['usos'] as $usos) {
    		    $usoss.= $usos['dosis'].' '. $usos['unidad_dosis'].', ';
    		}
    		echo rtrim ($usoss, ' ,' );
		echo '</td>';
		...
		echo'<td>';
			foreach ((array)$registros['usos'] as $usos) { 
			    //$usoss1.= 'Cultivo: ' .$usos['cultivo_nombre_comun'].' - '.$usos['cultivo_nombre_cientifico'].' aplicado a Plaga: '. $usos['plaga_nombre_comun'].' - '. $usos['plaga_nombre_cientifico']. ' con dosis: ' . $usos['dosis'].' '. $usos['unidad_dosis'].' con período carencia: '. $usos['periodo_carencia']. ' con gasto de agua: '. $usos['gasto_agua'].' '. $usos['unidad_gasto_agua'].', ';
			    $usoss1.= 'Cultivo: ' .$usos['cultivo_nombre_comun'].' - '.$usos['cultivo_nombre_cientifico'].' aplicado a Plaga: '. $usos['plaga_nombre_comun'].' - '. $usos['plaga_nombre_cientifico'].', ';
			}
			echo rtrim ($usoss1, ' ,' );
		echo '</td>';
		...
		echo'<td>';
			...
			if(count((array)$registros['manufacturador'])>0){
			    $formuladores.='Manufacturador: ';
    			foreach ((array)$registros['manufacturador'] as $manufacturador) {
    			    $formuladores.=$manufacturador['manufacturador'].' - '.$manufacturador['pais_origen'].', ';
    			}
			}
