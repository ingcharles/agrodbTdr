...
<form id="enviarAprobacion" data-rutaAplicacion="reformaPresupuestaria" data-opcion="enviarRevisionDGAFDGPGE" data-accionEnExito="ACTUALIZAR" >
	...
	<?php
		if($elementosPlanificacionAnual != 0){
			for ($i = 0; $i < count ($elementosPlanificacionAnual); $i++) {
				...
				$presupuestosRevisados = pg_fetch_result($crp->numeroPresupuestosRevisadosTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'revisado'), 0, 'num_presupuestos_revisados');
				...
				$presupuestosEnviadosRevisionGF = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'enviadoRevisorGF'), 0, 'num_presupuestos_revisados');
				$presupuestosEnviadosRevisionGA = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'enviadoRevisorGA'), 0, 'num_presupuestos_revisados');
				$presupuestosRevisadosGA = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'revisadoGA'), 0, 'num_presupuestos_revisados');
				$presupuestosEnviadosRevisionDGPGE = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'enviadoRevisorDGPGE'), 0, 'num_presupuestos_revisados');
				$presupuestosRevisadosDGPGE = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'revisadoDGPGE'), 0, 'num_presupuestos_revisados');
				...
				if(($planificacionAnual['estado'] == 'revisado') && (($total-$presupuestosAprobados) == ($presupuestosRevisados + $presupuestosEnviadosRevisionGF +
