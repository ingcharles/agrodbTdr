...
try {
	...
	$datosFicha = array(
			...
			'reposoDesde' => htmlspecialchars ($_POST['fechaReposoDesde'],ENT_NOQUOTES,'UTF-8'),
	        'reposoHasta' => htmlspecialchars ($_POST['fechaReposoHasta'],ENT_NOQUOTES,'UTF-8'));

	$cai->actualizarDatosFichaEmpleado($conexion, $datos['identificadorUsuario'], $datos['edad']);
	
	$consulta=$cai->modificarNuevoRegistroSso($conexion,$datos['solicitud'],$datos['escolaridad'],$datos['profesion'],$datos['horarioTrab'],
			$datos['tiempoPuesto'],$datos['opcion']);
	

	$cai->modificarInformeAccidente($conexion, $datos['solicitud'],$datos['diaSemana'],$datos['fechaSuceso'],$datos['horaAccidente'],$datos['tipoAccidente'],
	...
	$cai->modificarCircunstanciasAccidentes($conexion, $datos['solicitud'],$datos['describirAccidente'],$datos['trabajoHabitual'],$datos['accidenteTrabajo'],$datos['partesLesionadas'],
			$datos['personaAtendio'],$datos['trasladoAccidente'],$datos['nombreTestigo'],$datos['direccionTestigo'],$datos['telefonoTestigo']);
	
	if(pg_num_rows($cai->verificarDatosFichaMedica($conexion, $datos['solicitud'])) != 0)
	$cai->modificarFichaMedica($conexion, $datos['solicitud'],$datosFicha['lugarAtencion'],$datosFicha['fechaAtencion'],$datosFicha['horaAtencion'],$datosFicha['presentaSintomas'],
				$datosFicha['otrosDatos'],$datosFicha['descripcionLesiones'],$datosFicha['trasladoCentroSalud'],$datosFicha['nombreMedico'],$datosFicha['reposoDesde'],$datosFicha['reposoHasta']);
	else
	$cai->guardarFichaMedica($conexion, $datos['solicitud'],$datosFicha['lugarAtencion'],$datosFicha['fechaAtencion'],$datosFicha['horaAtencion'],$datosFicha['presentaSintomas'],
