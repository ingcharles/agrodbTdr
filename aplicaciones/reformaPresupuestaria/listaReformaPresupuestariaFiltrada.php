...
<body>
	...
	<?php  
		...
		$contador = 0;
		$res = $crp->listarProgramacionAnualTemporalVistaRevision($conexion, $areaN2, $areaN4, $idGestion, $tipo, 
															$anio, $identificadorRevisor, "'enviadoRevisor','revisado'");
		...
		while($fila = pg_fetch_assoc($res)){
			...
			if($presupuestosRechazados>0){
				...
				}else if(($fila['tipo']=='Proceso') && ($fila['estado']=='aprobadoDGPGE')){
				...
				}else if(($fila['tipo']=='Proyecto Gasto Corriente') && ($fila['estado']=='aprobadoDGPGE')){
					if($presupuestosEnviadosRevisor>0){
			...
			$contenido ='<article 
								...
								data-opcion="abrirReformaPlanificacionAnualRevision" 
