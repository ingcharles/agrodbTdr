	...
	if($tipo == 'certificacion'){
		...
		while($fila = pg_fetch_assoc($certificacionesBT)){
			...
			echo '<article 
						...
						data-opcion="abrirCertificacionBTLaboratorios" 
					...
					<aside><small><b> Fecha tentativa muestreo:</b>'.date('j/n/Y',strtotime($fila['fecha_nueva_inspeccion'])).'<br />
					</small></aside>
				</article>';
		}
	}else{
		...
		while($fila = pg_fetch_assoc($certificacionesBT)){
			...
			echo '<article
						...
						class="item"
						data-rutaAplicacion="certificacionBrucelosisTuberculosis"
						data-opcion="abrirRecertificacionBTLaboratorios"
						ondragstart="drag(event)"
						draggable="true"
						data-destino="detalleItem">
					<span class="ordinal">'.++$contador.'</span>
