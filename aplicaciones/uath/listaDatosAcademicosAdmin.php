...
?>
	<header>	
		...
		<nav>
		<form id="filtrar" data-rutaAplicacion="uath" data-opcion="listaDatosAcademicosAdmin" data-destino="areaTrabajo #listadoItems">
		...
		<table>
			...
			<?php 
				...
				$res = $cd->obtenerDatosAcadémicosAdmin($conexion, $identificador,$apellido,$nombre);
				$contador = 0;
				...
				if($areaUsuarioTH['clasificacion']=='Planta Central'){
					while($academico = pg_fetch_assoc($res)){	
						echo '<tr 	id="'.$academico['identificador'].'"
									class="item"
									data-rutaAplicacion="uath"
									data-opcion="revisaDatosAcademicosAdmin" 
									ondragstart="drag(event)" 
									draggable="true" 
									data-destino="detalleItem"
									>
								<td>'.++$contador.'</td>
								<td>'.$academico['identificador'].'</td>
								<td style="white-space:nowrap;"><b>'.$academico['apellido'].'</b></td>
					...
					while($academico = pg_fetch_assoc($res)){	
						if (in_array($academico['id_area'], $zonasFuncionarios)) {
							echo '<tr 	id="'.$academico['identificador'].'"
										class="item"
										data-rutaAplicacion="uath"
										data-opcion="revisaDatosAcademicosAdmin" 
