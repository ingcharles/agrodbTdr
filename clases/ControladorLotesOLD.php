...
class ControladorLotes{
	...
	public function listarProveedoresPorProducto($conexion,$idProducto,$idOperador){
		...
		$consulta="SELECT  
					...
					WHERE 		
						ope.identificador_operador = pr.codigo_proveedor and
						op.identificador = pr.codigo_proveedor and
						ope.estado in ('registrado','registradoObservacion') and 
						pr.identificador_operador='$idOperador'	
						and ope.id_producto='$idProducto' and pr.id_producto='$idProducto';";		
	...
	public function ObtenerRegistrosConformados($conexion,$idLote,$operador){
		...
		$consulta="SELECT
						r.id_registro, r.codigo_registro,
						case when op.razon_social = '' then op.nombre_representante ||' '|| op.apellido_representante else op.razon_social end nombre_proveedor, r.cantidad
			  		FROM
	...
	public function sitiosXidProductoAcopiador($conexion,$producto,$operador,$op=""){
		$consulta="SELECT 
						id_tipo_operacion, codigo, id_area
					FROM
						g_catalogos.tipos_operacion
		...
		$tipoOperacion= $fila['id_tipo_operacion'];
		if($op!="")
		$busqueda="and t.id_tipo_operacion='$tipoOperacion'";
