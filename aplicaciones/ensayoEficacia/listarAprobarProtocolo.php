...
<header>
	...
	<div id="aprobarProtocoloCor">
		<h2>Por aprobar protocolo del Coordinador</h2>
		<div class="elementos"></div>
	</div>
	<div id="inspeccion">
		<h2>En inspección</h2>
	...
	if($perfil->esDirector() ){
		$tramitesRecuperados=$ce->obtenerFlujosDeTramitesAsignarEE($conexion,$identificador,$flujo['id_fase'],$estado,$perfilActual,'N');
		...
		$estadoBuscado='aprobarProtocoloCor';
		$flujo=$ce->obtenerFlujo($conexion,$_SESSION['idAplicacion'],$estadoBuscado);
		$fase_flujo=$flujo['id_fase'];
		

		$tramitesRecuperados=$ce->obtenerFlujosDeTramitesAsignarEE($conexion,$identificador,$fase_flujo,$estadoBuscado,$perfilActual,'N');
		while($fila = pg_fetch_assoc($tramitesRecuperados)){
			$tramites[]=$fila;
		}
		$perfiles[]=$perfilActual;
	...
	if(sizeof($tramites)>0){
		...
		foreach($tramites as $key=>$fila)
		{
			...
			$contenido = '<article
							...
							data-opcion="abrirAprobacionProtocolo"
