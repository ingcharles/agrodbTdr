...
try{
	...
	switch($opcion_llamada){
		...
		case 'asignarTramiteDossier':
			...
			if($perfil->tieneEstePerfil('PFL_RES_CENTRAL')){
				...
				$dossier=$cg->obtenerSolicitud($conexion,$id_documento);
				try{
		...
		case 'guardarObservacionesSolicitud':
			...
			foreach($_POST as $key=>$item){
				...
				if(substr($key,0,7)=='obs_GI_'){
					$pendiente='O';	//Al menos una observacion en los ingredientes
				}
			}
			...
			if($perfil->tieneEstePerfil('PFL_DG_ARIA') ){
		...
		case 'guardarPuntosEtiqueta':
			...
			$respuesta=$cGenerador->generarPuntosMinimos($conexion,$id_solicitud,$firmante,$firmanteCargo);

			$datosEtiqueta=$cg->obtenerEtiquetaSolicitud($conexion, $id_solicitud);
			$datosEtiqueta['estado']='aprobarEtiqueta';
			$datosEtiqueta['ruta']=$respuesta['datos'];
			$res=$cg->guardarEtiqueta($conexion,$datosEtiqueta);

			$mensaje['estado'] = 'exito';
			$mensaje['mensaje'] = 'Generados puntos mínimos de etiqueta';
			break;
		...
		case 'guardarOrganismoExterno':
			...
			$res=$cg->guardarSolicitud($conexion,$datosDossier);
			$mensaje['estado'] = 'exito';
			$mensaje['mensaje'] = 'Trámite enviado para ser subsanado';
			break;
	...
	if($flujo!=null && $esta_procesado && $ident!=''){
		...
		try{
			...
			if($finTramite){
				//Genera Certificado
				$ccert=new GeneradorDocumentoPlaguicida();
				...
				array_push($adjuntos,$datosCertificado['datos']);
				array_push($adjuntos,$datosEtiqueta['datos']);
			}
			$cg -> guardarSolicitud($conexion,$datosDocumento);
			if($generarDossier){
				$cGenerador=new GeneradorDocumentoPlaguicida();
				$respuesta=$cGenerador->generarDossier($conexion,$datosDocumento['id_solicitud']);
