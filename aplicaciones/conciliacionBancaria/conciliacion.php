...
$anioProcesoConciliacion =  $_POST['anioProcesoConciliacion'];
$mesProcesoConciliacion =  str_pad($_POST['mesProcesoConciliacion'], 2, "0", STR_PAD_LEFT);
$diaProcesoConciliacion =  str_pad($_POST['diaProcesoConciliacion'], 2, "0", STR_PAD_LEFT);
$totalRecaudado =  $_POST['totalRecaudado'];*/

$idProcesoConciliacion = 24;
$anioProcesoConciliacion = 2017;
$mesProcesoConciliacion = 12;
$diaProcesoConciliacion = 19;
$totalRecaudado = 40050;
...
$fechaConciliacion = ($diaProcesoConciliacion.'/'.$mesProcesoConciliacion.'/'.$anioProcesoConciliacion);
...
$rutasProcesoConciliacion = $_POST['hRutaArchivo'];
	...
	//Fin variables del proceso de conciliación
	...
	$qProcesoConciliacion = $cb-> abrirRegistroProcesoConciliacionXIdProcesoConciliacion($conexion, $idProcesoConciliacion);
	...
	$qCamposOrdenPagoGuia = $cf-> abrirCamposOrdenPagoXFechaFacturacion($conexion, $fechaConciliacion);
	...
	$arrayGuiaTP= array();
	$arrayGuiaTS= array();
	$arrayGuiaTP+= array('facturaGuia'=>'Factura GUIA');
	...
	$arrayOrdenesGeneradasGuia = $cf-> abrirOrdenPagoPorFechaConciliacion ($conexion, $fechaConciliacion);
	...
	echo "<br>numero_facturasguia:".$numeroFacturasGuia."<br>";
	...
	switch($ordenesGuiaVue){
		...
		case true: // SE verifica si existen datos en GUIA Y TRAMA			
			...
			foreach ($arrayOrdenesGeneradasGuia as $keyOrdenesGeneradasGuia => $valorOrdenesGeneradasGuia){
				...
				if($contadorGuia == 1){
					...
					$arrayGuiaTS['facturaGuia']= array('numero_factura'=>'numero_factura_guia','numero_orden_vue'=>'numero_orden_pago_factura','total_pagar'=>'monto_recaudacion', 'tipo_liquidacion'=>'tipo_liquidacion', 'fecha_hora_pago_vue'=>'fecha_hora_pago_vue');
			...
			while ($rutaDocumento = pg_fetch_assoc($qRutaDocumento)){
				...
				echo $ruta = $rutaDocumento['ruta_documento_proceso_conciliacion'];
				...
				switch ($tipoDocumento){//Verifica si es documento o trama
					...
					case 'trama':
						...
						$arrayTramas[] = array('tipoDocumento'=>$tipoDocumento, 'formatoDocumento'=>$formatoDocumento, 'rutaDocumento'=>$ruta, 'nombreDocumento'=>$nombreDocumento); //arrayTramas
						...
						switch ($formatoDocumento){//Verifica el formato del documento .txt
							...
							case 'txt':
								...
								$arrayDetalleTP+= array('trama'=>$nombreDocumento);
								$arrayDetalleTP+= array('resumen'=>'Resumen trama');
								...
								$arrayGeneralTP+= array('lecturaTrama'=>'Lectura documento de trama');
								...
								$tramasNoConciliadas = fopen("documentos/tramasNoConciliadas.txt","r");
									...
									if ($linea = fgets($archivo)){										
										...
										if($tipoFila!==FALSE){
												...
												switch($tipoCampo){
													...
													case 'campoDetalle':
														...
														if($posicion!==FALSE && $posicion == 0){
															...
															echo $valorCampo."<br>";
															$arrayCampos+= array($campoComparar=>$valorCampo);
											...
											if($banderaValorEncontrado){
												...
												$arrayCampos+=array('estado'=>'no conciliado'/*,'diferencia'=>''*/);
											...
											foreach ($qDatosCamposDetalle as $keyTramaDetalle => $valorTramaDetalle){
												...
												if($posicionDetalle!==FALSE && $posicionDetalle == 0){
													...
													$arrayCamposDetalle+= array($campoDetalle=>$valorCampoDetalle);
								...
								switch ($banderaNoHaytramasGuiaX){
								...
								foreach ($arrayFilaCampos as $keyFilaTrama=>$valorFilaTrama){
									...
									$qCamposOrdenPago = $cf-> abrirCamposOrdenPagoXFechaFacturacionXNumeroOrdenVue($conexion, $fechaConciliacion, $valorFilaTrama['numero_orden_vue']);
									...
									$arrayDetalleTS['resumen']= array('estadoTrama'=>'Estado'/*, 'diferencias'=>'Diferencias'*/);
							...
							echo "Es un archivo xls";
								
							break;
							
							case 'csv':
								
							echo "Es un archivo csv";
								
							break;
							
							case 'xml':
								
							echo "Es un archivo xml";
					...
					case 'documento':
						...
						$arrayDocumentos[] = array('tipoDocumento'=>$tipoDocumento, 'formatoDocumento'=>$formatoDocumento, 'rutaDocumento'=>$ruta, 'nombreDocumento'=>$nombreDocumento);
						...
						$arrayDetalleTP2+= array('documento'=>$nombreDocumento);
						$arrayDetalleTP2+= array('resumen'=>'Resumen Banco');
						...
						if (!file_exists($archivo)) {
							...
							for($i=$filaInicioLectura;$i<=$highestRow;$i++){
								...
								foreach ($qDatosCamposCompararDocumento as $keyDocumento => $valorDocumento){
									...
									$arrayCamposDocumento+= array($campoComparar => $valorExcel);
								...
								if($banderaValorEncontrado){
									...
									$arrayCamposDocumento+=array('estado'=>'no conciliado'/*,'diferencia'=>''*/);
							...
							/*echo "<pre>";
							print_r($banderaNoHayDocumentosGuia);
							echo "<pre>";*/
							...
							switch($banderaNoHayDocumentoGuiaX){
							...
							foreach ($arrayFilaCamposDocumento as $keyFilaCampo=>$valorFilaCampo){
								...
								$qCamposOrdenPago = $cf-> abrirCamposOrdenPagoXFechaFacturacionXNumeroOrdenVue($conexion, $fechaConciliacion, $valorFilaCampo['numero_orden_vue']);
								...
								$arrayDetalleTS2['resumen']= array('estadoBanco'=>'Estado'/*, 'diferencias'=>'Diferencias'*/);
			...
			for($h=1; $h<=$contadorGeneral; $h++){
				...
				foreach (${arrayPrincipalGeneral.$contadorTrama}['general']['tituloPrincipal'] as $keyPrincipal =>$valorPrincipal){
					...
					array_push($arrayTablaGeneral, array($valorPrincipal));
				}
			...
			foreach($arrayOrdenesGuia as $keyOrden => $valueOrden){
				...
				if($j==1){
					...
					foreach ($arrayPrincipalGuia['guia']['tituloSecundario'] as $keySecundarioTS =>$valorSecundarioTS){
						...
						$colspanTituloSecundario+=array($keySecundarioTS=>count($valorSecundarioTS));
					...
					for($h=1; $h<=$contadorTrama; $h++){
						...
						foreach (${arrayPrincipalTrama.$h}['campoDetalle']['tituloSecundario'] as $keyTramaTS =>$valorTramaTS){
							...
							$colspanTituloSecundario+=array($keyTramaTS=>count($valorTramaTS));
						...
						foreach (${arrayPrincipalDocumento.$h}['documentoDetalle']['tituloSecundario'] as $keyDocumentoTS =>$valorDocumentoTS){
							...
							$colspanTituloSecundario+=array($keyDocumentoTS=>count($valorDocumentoTS));
						...
						if($keyOrden == 'numero_orden_vue'){
							...
							foreach ($arrayTramas as $keyArchivo => $valueArchivo){
								...
								if($tipoArchivo == "documento" && $formatoArchivo == "xls"){
									...
									/*$lineas = file("documentos/1722551049_20170920182937_SENAE-Banred.txt");
		...
		//////GNERAR EXCEL
			...
			$styles1 = array( 'border'=>'left,right,top,bottom', 'halign'=>'center', 'fill'=>'#1a60aa', 'color'=>'#ffffff', 'font-style'=>'bold');
			$styles2 = array( 'border'=>'left,right,top,bottom');
			
			//Insertar registros de hoja 1
			$valorColumna= 0;
			$header = array("string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string","string");
			$writer->writeSheetHeader($sheet1, $header, $col_options = array('widths'=>[25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25], 'suppress_row'=>true));
			$writer->writeSheetHeader($sheet2, $header, $col_options = array('widths'=>[25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25], 'suppress_row'=>true));
			$writer->writeSheetHeader($sheet3, $header, $col_options = array('widths'=>[25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25], 'suppress_row'=>true));
			$writer->writeSheetHeader($sheet4, $header, $col_options = array('widths'=>[25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25,25], 'suppress_row'=>true));
			...
			$hora= time();
			$rutaArchivoConciliacion = 'reportesConciliaciones/conciliacionXXX'.$hora.'.xlsx';
			...
			/*echo '<pre>';
			 print_r(${arrayPrincipalTrama.$h}['campoDetalle']);
			echo '<pre>';*/
			...
			foreach ($arrayNumeroVue as $keyNumeroVue => $valueNumeroVue){
				...
				foreach ($arrayDocumentos as $keyArchivo => $valueArchivo){
					...
					$archivo1 = fopen ("../../".$rutaArchivo, "r");
			
					while (!feof ($archivo1)) {
			
						if ($linea1 = fgets($archivo1)){
			
							$tipoFila1 = strpos($linea1, $datosDetalle['codigo_segmento_detalle_trama']);
							...
							if($tipoFila1!==FALSE){
								...
								foreach ($qDatosCamposDetalle as $keyFilaTramaDetalle=>$valorFilaTramaDetalle){//TODO:ESTA BIEN
									...
									$tamanioDetalle = strlen($codigoSegmentoDetalle);//Obtiene el tamanio del codigo del segmento
									$posicionDetalle = strpos($linea1, $codigoSegmentoDetalle);
										
									if($posicionDetalle!==FALSE && $posicionDetalle == 0){
			
										$valorCampoDetalle = substr($linea1,($posicionInicialDetalle-1), $longitudSegmentoDetalle);
			
										$catalogoCampo = $cb -> abrirDatoscatalogosCampos($conexion, $idCampoDetalle, $valorCampoDetalle);
			
										$qIdPagoFecha = pg_fetch_assoc($cb -> buscarIdPagoYFechaXIdVue($conexion, $valueNumeroVue));
										...
										switch($formaPago){
											...
											case 'valorDeposito':
												$valorDeposito = ($valorCampoDetalle=='') ? 0 : $valorCampoDetalle;
												break;
								...
								if(pg_num_rows($cb -> buscarIdPagoFormaPago($conexion, $idPago))==0){
									...
									$cb -> insertarFormaPagoGuia($conexion, $idPago, $idBanco, $banco, $transaccion, 0, $fechaOrdenPago, 0, $idCuentaBancaria, $numeroCuenta);
			
								}else{
			
									$cb -> actualizarFormaPagoGuia($conexion, $idPago, $idBanco, $banco, $transaccion, 0, $fechaOrdenPago, 0, $idCuentaBancaria, $numeroCuenta);
	...
	/*if($imprimirArray && $banderaNoHaytramasGuia && $banderaNoHayDocumentosGuia){
		...
		echo '<pre>';
		print_r(${arrayPrincipalTrama.$contadorDocumento});
		echo '<pre>';
