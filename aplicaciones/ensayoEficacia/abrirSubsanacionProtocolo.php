...
session_start();
	...
	$id_fase=$ce->obtenerFaseDelFlujo($conexion,$id_flujo,'subsanarProtocolo');
	$resultado=$ce->obtenerFlujosDeTramitesProtocoloEE($conexion,$id_fase,$identificador,$id_documento);
	$tramite_flujo=pg_fetch_assoc($resultado,0);
	$id_tramite_flujo = $tramite_flujo['id_tramite_flujo'];
	
	$observaciones=$ce->obtenerObservacionesDelDocumento($conexion,$id_documento);
...
<script type="text/javascript">
	...
	$("document").ready(function(){
		...
		$('#detalleItem').find('h1').first().html("Subsanación del protocolo");
