	...
	try{
		...
		try {
			...
			if(($identificador != null) || ($identificador != '')){
				...
				$inventarioAnimal = $cbt->buscarInventarioAnimalCertificacionBT($conexion, $idCertificacionBT, $idAnimalesPredio, $numInspeccion);
		
				if(pg_num_rows($inventarioAnimal) == 0){
					$conexion->ejecutarConsulta("begin;");
					
					$idInventarioAnimal = pg_fetch_result($cbt->guardarInventarioAnimalCertificacionBT($conexion, 
																	$idCertificacionBT, $identificador,
																	$idAnimalesPredio, $animalesPredio, $numeroAnimalesPredio,
																	$numInspeccion), 
																	0, 'id_certificacion_bt_inventario_animal');				
					
					$mensaje['estado'] = 'exito';
					$mensaje['mensaje'] = $cbt->imprimirLineaInventarioAnimalCertificacionBT($idInventarioAnimal,  
