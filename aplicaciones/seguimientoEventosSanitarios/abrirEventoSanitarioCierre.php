<?php 
	...
	$vacunacionFinalesConsulta = $cpco->listarVacunacionFinales($conexion, $idEventoSanitario);
        $vacunacionFinalesConsulta2 = $cpco->listarVacunacionFinales($conexion, $idEventoSanitario);
        
        $cont = 0;
					while ($vacuna = pg_fetch_assoc($vacunacionFinalesConsulta2)){
                                            $cont+=1;
						
					}
                                                if($cont>0) {
            $vacunacionFinalSiNo = 'Si';
        } else {
            $vacunacionFinalSiNo = 'No';
        }
