...
try{
	...
	try {
		...
		$ca ->guardarEliminar($conexion, pg_fetch_result($qTransaccion, 0, 'id_transaccion'),$_SESSION['usuario'],'El usuario <b>' . $_SESSION['datosUsuario'] . '</b> ha eliminado el código complementario '.$idCodigoComplementario.' y código suplementario '.$idCodigoSuplementario.' del producto con código '.$idProducto);
