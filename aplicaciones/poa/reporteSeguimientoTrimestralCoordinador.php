...
$datosProceso = $cpoa1->obtenerSubprocesoXProceso($conexion);
...
$datosProceso2= $cpoa1->obtenerComponenteXProceso($conexion);
...
$datosProceso3= $cpoa1->obtenerActividadesXSubProceso($conexion);
...
$areaCoordinador = $ca->areaUsuario($conexion, $_SESSION['usuario']);
?>

<header>
	<h1>Reporte matriz poa</h1>
	<nav>
		<form id="filtrar" action="aplicaciones/poa/reporteMPOAConsolidadoSeguimientoTrimestral.php" target="_blank" method="post">
			<table class="filtro">
				<tr>
					<th>Que contenga</th>
					<td>Dirección:</td>
					<td><select name="areaDireccion" id="areaDireccion">
							...
							while($fila = pg_fetch_assoc($res)){
								if(pg_fetch_result($areaCoordinador, 0, 'id_area') == $fila['id_area']){
				...
				?>
					...
					<td><select id="listaObjetivoEstrategico"
						name="listaObjetivoEstrategico">
							...
							<?php 
							$res= $cpoa1->listarObjetivosEstrategicos($conexion);
				...
				<tr>
					...
					<td><select id="listaProcesos" name="listaProcesos">
							<option value="">Todos</option>
							<?php 
							$res= $cpoa1->listarProcesos($conexion);
					...
					<td><select id="listaSubprocesos" name="listaSubprocesos">
							<option value="">Todos</option>
				<?php 
				$res= $cpoa1->listarSubprocesos($conexion);
				...
				<tr>
					...
					<td><select id="listaComponentes" name="listaComponentes">
							<option value="">Todos</option>
				<?php 					
				$res= $cpoa1->listarComponentes($conexion);
				...
				while($fila = pg_fetch_assoc($res)){
					...
					<td>Actividades:</td>
					<td><select id="listaActividades" name="listaActividades">
							<option value="">Todos</option>
				<?php 
					$res= $cpoa1->listarActividades($conexion);
    				while($fila = pg_fetch_assoc($res)){
	                       echo '<option value="' . $fila['descripcion'] ."". $fila['id_proceso'] . '">' . $fila['descripcion'] .'</option>';
					}?>
			</select></td>
	...
	$("#listaSubprocesos").change(function(){
	    ...
	    for(var z=0;z<array_actividades.length;z++){
		    ...
		    if ($("#listaSubprocesos").val()==array_actividades[z]['id_subproceso']){
		    	sactividades += '<option value="'+array_actividades[z]['descripcion_actividad']+array_actividades[z]['id_subproceso']+'">'+array_actividades[z]['descripcion_actividad']+'</option>';
			    }
	   		}
