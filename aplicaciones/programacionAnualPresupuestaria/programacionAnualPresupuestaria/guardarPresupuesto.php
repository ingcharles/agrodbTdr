...
try{
	...
	try {
		...
		$presupuesto = $cpp->buscarPresupuesto($conexion, $detalleGasto, $cuatrimestre, $idPlanificacionAnual);
		
		if(pg_num_rows($presupuesto) == 0){
			$conexion->ejecutarConsulta("begin;");
			$idPresupuesto = pg_fetch_result($cpp->nuevoPresupuesto($conexion, $idPlanificacionAnual, $ejercicio, $entidad,
					...
					$proyectoBID, $idTipoRegimen, $tipoPresupuesto, $agregarPac, $iva, $costoIva, $idRevisor, $idAreaRevisor, $anio, $idAreaFuncionario, $identificador),
					0, 'id_presupuesto');
			$conexion->ejecutarConsulta("commit;");
		
			$mensaje['estado'] = 'exito';
			$mensaje['mensaje'] = $cpp->imprimirLineaPresupuesto($idPresupuesto, $nombreActividad, $detalleGasto, $renglon,
					 $costoIva, $cantidadAnual, $cuatrimestre, $idPlanificacionAnual, 'programacionAnualPresupuestaria', 
					null, 'creado');
		}else{
			$mensaje['estado'] = 'error';
