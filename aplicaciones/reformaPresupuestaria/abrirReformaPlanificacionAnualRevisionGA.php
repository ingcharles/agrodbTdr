<?php 
	...
	$presupuesto = $crp->listarPresupuestosTemporalesXEstado($conexion, $idProgramacionAnual,"enviadoRevisorGA', 'revisadoGA', 'rechazado");
	
	/*$total = pg_fetch_assoc($crp->numeroPresupuestosReformadosTemporal($conexion, $idPlanificacionAnual, "enviadoRevisorGA', 'revisado"));
	...
	$presupuestosEnviadosRevisor = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $idProgramacionAnual, 'enviadoRevisorGA'), 0, 'num_presupuestos_revisados');
				...
				<?php 
					while ($presupuestos = pg_fetch_assoc($presupuesto)){
						echo $crp->imprimirLineaPresupuestoRevisionDGAFDGPGE($presupuestos['id_presupuesto'], $presupuestos['nombre_actividad'], $presupuestos['detalle_gasto'], 
															...
															$presupuestos['revisado'],'GA', $presupuestos['estado']);
					}
				?>
			</table>
		</fieldset>
