	...
	<fieldset>
		...
		<div data-linea="5">
			<label for="cbPlantilla">Plantilla:</label>
				<select id="cbPlantilla" name="cbPlantilla" >
				<?php
				    $res=$cl->obtenerPlantillasRuta($conexion, 1);					   
				    while($fila=pg_fetch_assoc($res)){
				        echo '<option value="'.$fila['codigo'].'" data-ruta="'.$fila['ruta'].'" >'.$fila['nombre'].'</option>';
				    }				    
				?>
...
$("#cbPlantilla").change(function(event){
	$("#resultadoPlantilla").html('<div style="text-align:center;width:100%;padding-bottom: 10px;"><img src="'+$("#cbPlantilla option:selected").attr('data-ruta')+'" width="250" height="150"></div>');
});
...
$("#btnPrevizualizar").click(function(event){
	...
	switch($("#cbTamanio").val()){
		case'etiqueta':			
				contenido+='<div style="text-align:center;width:100%;padding-bottom: 10px;"><img src="'+rutaImagen+'" width="250" height="150"></div>';			
		...
		case'A4':
			...
			$("#vizualizador").html('<div id="hoja"></div>');
				for(i=0;i<=val-1;i++){
					contenido+='<div style="text-align:left;width:100%;padding-bottom: 10px;"><img src="'+rutaImagen+'" width="250" height="150"></div>';
				}
			$("#hoja").html(contenido);	
			} else{
				...
				for(i=0;i<=val-1;i++){			
					contenido+='<div style="float:left; padding-bottom: 10px; margin-right:20px"><img src="'+rutaImagen+'" width="250" height="150"></div>';					
				}				
