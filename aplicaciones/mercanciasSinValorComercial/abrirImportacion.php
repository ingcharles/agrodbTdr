...
<header>
<h1>Editar solicitud de Importación</h1>
</header>
	...
	echo '<fieldset>
		...
		if($filaSolicitud['estado']=="subsanacion"){
			echo '<div data-linea="1"><label>*Tipo de identificación: </label><select id="tipoIdentificacion" name="tipoIdentificacion" disabled="disabled"><option value = "">Seleccione....</option>
				...
				<div data-linea="5"><label>*Teléfono: </label><input type="number" id="telefonoPropietario" name="telefonoPropietario" value="'.$filaSolicitud['telefono_propietario'].'" disabled="disabled" class="soloNumeros"/></div>
...
?>
	<fieldset>
		...
		<div data-linea="2">
			<?php 
				echo '<label for="pais">'.$obligatorio.' País Origen: </label>';
				if($filaSolicitud['estado']!="subsanacion"){
		...
		<div data-linea="3" id="resultadoEmbarque">
			<?php
				echo '<label for="puertoEmbarque">'.$obligatorio.' Puerto Embarque: </label>';
				if($filaSolicitud['estado']!="subsanacion"){
					echo $filaSolicitud['nombre_puerto'];
				} else{
					$embarque=$cc->listarPuertosPorPais($conexion, $filaSolicitud['id_localizacion_origen_destino']);
					echo '<select id="puertoEmbarque" name="puertoEmbarque" disabled>
						<option value="">Seleccione...</option>';
						while($fila=pg_fetch_assoc($embarque)){
							echo'<option value="'.$fila['id_puerto'].'">'.$fila['nombre_puerto'].'</option>';
						}
					echo '</select>
							<input type="hidden" id="nombrePuerto" name="nombrePuerto" value="'. $filaSolicitud['nombre_puerto'].'">';
				}
			?>
		</div>
		<div data-linea="4">
			<?php
				echo '<label for="residencia">'.$obligatorio.' Dirección Residencia en Ecuador: </label>';
				if($filaSolicitud['estado']!="subsanacion"){
					echo $filaSolicitud['direccion_ecuador'];
				}else{
					echo '<input type="text" id="residencia" name="residencia" value="'.$filaSolicitud['direccion_ecuador'].'" disabled >';
				}
			?>
		</div>
		<div data-linea="5">
		...
		<div data-linea="5">
			<?php
				echo '<label for="medioTransporte">'.$obligatorio.' Medio Transporte: </label>';
				if($filaSolicitud['estado']!="subsanacion"){
					$fila = pg_fetch_assoc($res= $cc->listarMediosTrasporteXid($conexion, $filaSolicitud['id_medios_transporte']));
					echo $fila['tipo'];
				}else{
					echo '<select id="medioTransporte" name="medioTransporte" value="'.$filaSolicitud['id_medios_transporte'].'" disabled>';
						echo '<option value="">Seleccione...</option>';
		...
		<div data-linea="8">
			<?php
				...
				echo '</select>
						<input type="hidden" name="nombreUso" id="nombreUso" value="'. $filaSolicitud['nombre_uso'].'">';
				}
			?>
		</div>
		<div data-linea="9">
	...
	if($filaSolicitud['estado']=="subsanacion"){
		...
		<hr>
		<div style="width:100%">
			<label>'.$obligatorio.'Certificado Zoosanitario de Exportación:</label>
			<input type="hidden" class="rutaArchivo" id="rutaZoosanitario" name="rutaZoosanitario" value="'.$filaDocumento['ruta_zoosanitario_exp'].'" />
			<input type="file" class="archivo" id="archivoZoosanitario" accept="application/msword | application/pdf | image/*" disabled />
...
<form id="nuevoRegistro" data-rutaAplicacion="mercanciasSinValorComercial" data-opcion="guardarNuevoProducto">
		...
		<fieldset id="datosProducto">
			...
			<div data-linea="8">
				...
				<input type="text" id="edad" name="edad" disabled="disabled" required="required" class="soloNumeros">
			</div>		
			<div data-linea="9">
...
$("document").ready(function(){
	...
	cargarValorDefecto("puertoEmbarque","<?php echo $filaSolicitud['id_puerto'];?>");
	cargarValorDefecto("medioTransporte","<?php echo $filaSolicitud['id_medios_transporte'];?>");
...
$("#modificar").click(function (event){
	...
	$("#archivoZoosanitario").removeAttr("disabled");
	...
	$("#medioTransporte").removeAttr("disabled");
...
$("#pais").change(function (event){	
	if($.trim($("#detalleItem #pais").val()) != "" ){
		...
		$("#cargarSolicitud").attr('data-destino', 'resultadoEmbarque');
	    $("#cargarSolicitud").attr('data-opcion', 'comboImportacion');
		$("#cargarSolicitud #opcion").val("puertos");
		abrir($("#cargarSolicitud"), event, false);
	}
});

$("#puertoEmbarque").change(function(event){
	$("#nombrePuerto").val($("#puertoEmbarque option:selected").text());
});
...
$("#tipoProducto").change(function(event){
	if($("#tipoProducto").val()!=""){
		...
		$("#nuevoRegistro #opcion").val("subtipo");
