...
try{
	...
	$idPresentacion = htmlspecialchars($_POST['idPresentacion'],ENT_NOQUOTES,'UTF-8');
	...
	try {
		...
		$cr->actualizarEstadoPresentacion($conexion, $idPresentacion, $estado, $identificador);
		...
		$ca ->guardarUpdate($conexion, pg_fetch_result($qTransaccion, 0, 'id_transaccion'),$_SESSION['usuario'],'El usuario <b>' . $_SESSION['datosUsuario'] . '</b> ha modificado el estado de la presentación '.$presentacion.' a '.$estado);
		
		/*FIN AUDITORIA*/
		
		$mensaje['estado'] = 'exito';
		$mensaje['mensaje'] = $idPresentacion;
