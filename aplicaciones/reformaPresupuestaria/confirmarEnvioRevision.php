...
<form id="enviarRevision" data-rutaAplicacion="reformaPresupuestaria" data-opcion="enviarRevision" data-accionEnExito="ACTUALIZAR" >
	<input type='hidden' id='identificador' name='identificador' value="<?php echo $identificador;?>" />
	
	<p>La siguiente <b>Reforma Presupuestaria</b> será enviada a revisión: </p>
 	
	<?php
		if($elementosPlanificacionAnual != 0){
			for ($i = 0; $i < count ($elementosPlanificacionAnual); $i++) {
				$planificacionAnual = pg_fetch_assoc($crp->abrirProgramacionAnualTemporal($conexion, $elementosPlanificacionAnual[$i], $identificador));
				...
				$presupuestosRechazados = pg_fetch_result($crp->numeroPresupuestosRevisadosTemporal($conexion, $planificacionAnual['id_planificacion_anual'], 'rechazado'), 0, 'num_presupuestos_revisados');
			...
			if(($planificacionAnual['estado'] == 'enviadoRevisorGA') && ($presupuestosRechazados > 0)){
					echo'<fieldset>
							<legend>Código N° </label>' .$planificacionAnual['id_planificacion_anual'].'</legend>
								<div data-linea="1">
									<label>Gestión/Unidad: </label>' .$planificacionAnual['gestion'].'
			...
			if(($planificacionAnual['estado'] == 'enviadoRevisorGF') && ($presupuestosRechazados > 0)){
				echo'<fieldset>
						<legend>Código N° </label>' .$planificacionAnual['id_planificacion_anual'].'</legend>
							<div data-linea="1">
								<label>Gestión/Unidad: </label>' .$planificacionAnual['gestion'].'
