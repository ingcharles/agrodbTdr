<?php
/**
 * Modelo TecnicoModelo
 *
 * Este archivo se complementa con el archivo TecnicoLogicaNegocio.
 *
 * @author AGROCALIDAD
 * @date    2021-10-18
 * @uses TecnicoModelo
...
class TecnicoModelo extends ModeloBase{
	 ...
	 */
	private $tabla = "tecnico";

	/**
	 * Clave primaria
	 */
	private $clavePrimaria = "id_tecnico";

	/**
	 * Secuencia
	 */
	private $secuencial = 'g_registro_control_documentos"."tecnico_id_tecnico_seq';
	...
	public function __set($name, $value){
		...
		if (! method_exists($this, $method)){
			throw new \Exception('Clase Modelo: TecnicoModelo. Propiedad especificada invalida: set' . $name);
		}
		$this->$method($value);
	}
	...
	public function __get($name){
		$method = 'get' . $name;
		if (! method_exists($this, $method)){
			throw new \Exception('Clase Modelo: TecnicoModelo. Propiedad especificada invalida: get' . $name);
		}
		return $this->$method();
	}
