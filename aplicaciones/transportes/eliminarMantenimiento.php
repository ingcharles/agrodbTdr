	...
	try {
		...
		if ($identificadorUsuarioRegistro != ''){
			for ($i = 0; $i < count ($mantenimiento); $i++) {
				$cv->darBajaMantenimiento($conexion, $mantenimiento[$i],$datos['observacion'],$identificadorUsuarioRegistro);
				$cv->actualizarEstadoVehiculo($conexion, $placa[$i], 'Liberar');
			}
