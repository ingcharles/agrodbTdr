	...
	try{
		...
		try {
			...
			if(($identificador != null) || ($identificador != '')){
				...
				$numero = pg_fetch_result($cbt->generarNumeroMuestraRecertificacionBT($conexion, $numeroSolicitud), 0, 'num_muestra');
				$incremento = ($numero)+1;
				$numeroMuestra = str_pad($incremento, 4, "0", STR_PAD_LEFT);
					
				$idResultadoLaboratorio = pg_fetch_result($cbt -> nuevoResultadoLaboratorioRecertificacion($conexion, $idRecertificacionBT, $identificador, $numeroSolicitud, $numeroMuestra, 
													$resultadoAnalisisLaboratorio, $informe, $observaciones, $numInspeccion), 0, 'id_recertificacion_bt_resultado_laboratorio');
				
				for ($i = 0; $i < count ($muestra); $i++) {
					$cbt -> nuevoDetalleResultadoLaboratorioRecertificacion($conexion, $idResultadoLaboratorio, $idRecertificacionBT, 
				...
				if($resultadoAnalisisLaboratorio == "negativo"){
					...
					$numInspeccionC = pg_fetch_result($cbt->generarNumeroInspeccionRecertificacionBT($conexion, 'Inspección ', $numeroSolicitud), 0, 'num_inspeccion');
					...
					$cbt->cambiarEstadoRecertificadoBT($conexion, $idRecertificacionBT, $identificador, 'inspeccion', $numInspeccion);

				}else{
					$cbt->cambiarEstadoRecertificadoBT($conexion, $idRecertificacionBT, $identificador, 'plantaCentral', $numInspeccion);
				}
