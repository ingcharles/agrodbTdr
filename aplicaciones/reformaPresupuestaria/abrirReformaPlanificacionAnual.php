<?php 
	...
	$programacionAnual = pg_fetch_assoc($crp->abrirProgramacionAnualTemporal($conexion, $idProgramacionAnual, $identificador));
	...
	$responsable = $cu->listarUsuariosXArea($conexion, $programacionAnual['id_area_unidad']);
	
	$presupuesto = $crp->listarPresupuestosTemporales($conexion, $idProgramacionAnual);
	
	$total = pg_fetch_assoc($crp->numeroPresupuestosYCostoTotalIVATemporal($conexion, $idProgramacionAnual));
	?>
	...
	<div class="pestania">
		<form id="nuevoPresupuesto" data-rutaAplicacion="reformaPresupuestaria" data-opcion="guardarPresupuesto" data-destino="detalleItem">
			...
			<fieldset>
					...
					<select id=renglon name="renglon" required="required" >
						...
						<?php 
							...
							while($fila = pg_fetch_assoc($renglon)){
								echo '<option value="' . $fila['id_renglon'] . '" data-codigo="' . $fila['codigo'] . '" >' . $fila['codigo'] .' - '. $fila['nombre'].' </option>';
							}
						?>
					</select>
					...
					<select id=cpc name="cpc" required="required" >
						...
						<?php 
							...
							while($fila = pg_fetch_assoc($cpc)){
								echo '<option value="' . $fila['id_cpc'] . '" data-codigo="' . $fila['codigo'] . '">' . $fila['codigo'] .' - '. $fila['nombre']. ' </option>';
							}
						?>
					</select>
				...
				<div data-linea="12">
					<label>Cantidad Anual por Cuatrimestre: 1</label> 
			...
			<table id="detallePresupuestos">
				...
				<?php 
					while ($presupuestos = pg_fetch_assoc($presupuesto)){
						echo $crp->imprimirLineaPresupuesto($presupuestos['id_presupuesto'], $presupuestos['nombre_actividad'], $presupuestos['detalle_gasto'], 
															$presupuestos['renglon'], $presupuestos['costo_iva'], $presupuestos['cantidad_anual'], $presupuestos['cuatrimestre'], 
															$presupuestos['id_planificacion_anual'], 'reformaPresupuestaria', $presupuestos['revisado'], $presupuestos['estado']);
					}
				?>
			</table>
		</fieldset>
	...
	var estadoProgramacionAnual = <?php echo json_encode($estadoProgramacionAnual); ?>;
		...
		$("document").ready(function(){
			...
			if((estadoProgramacionAnual == 'creado') || (estadoProgramacionAnual == 'aprobado') || (estadoProgramacionAnual == 'rechazado')){
		...
		$("#tipoCompra").change(function (event) {
			...
			$("#nuevoPresupuesto").attr('data-rutaaplicacion', 'reformaPresupuestaria');
			$("#nuevoPresupuesto").attr('data-opcion', 'combosReformaPlanificacionAnual');
