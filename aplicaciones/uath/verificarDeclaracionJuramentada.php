...
try{
	...
	$declaracion_seleccionado=$_POST['declaracion_seleccionado'];
	$n        = count($estado);
	$i        = 0;
		...
		try {
				...
				while ($i < $n) 
				{
					$cc->modificarDeclaracionJuramentada($conexion, $declaracion_seleccionado[$i], '', $observaciones[$i],$estado[$i],'','');
					if(strcmp($estado[$i],'Rechazado') == 0 ){
						$fila=pg_fetch_assoc($cc->obtenerDatosUsuarioAgrocalidad($conexion,$_POST['identificadorServidor']));
						$asunto = 'VALIDACIÓN DE INFORMACIÓN EN DECLARACIÓN JURAMENTADA';
						...
						if($mailDestino != ''){
							...
							$tablaModulo = 'g_uath.datos_declaracion_juramentada';
					
							$qGuardarCorreo=$cMail->guardarCorreo($conexion, $asunto, $cuerpoMensaje, 'Por enviar', $codigoModulo, $tablaModulo, $declaracion_seleccionado[$i]);
