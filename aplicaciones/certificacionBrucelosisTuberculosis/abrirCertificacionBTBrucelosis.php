	...
	<form id="modificarCertificacionBT" data-rutaAplicacion="certificacionBrucelosisTuberculosis" data-opcion="modificarCertificacionBT" data-destino="detalleItem" >
		...
		<div id="actualizacion">
			...
			<fieldset>
				...
				<div data-linea="13">
					<label>Num. Cert. Fiebre Aftosa:</label>
					<input type="text" id="numCertFiebreAftosa" name="numCertFiebreAftosa" maxlength="32" data-er="^[0-9-]+$" value="<?php echo $certificacionBT['numero_certificado_fiebre_aftosa'];?>" data-inputmask="'mask': '9999-999-9999999'"/>
				</div>
...
<div class="pestania">
	...
	<form id="nuevoManejoAnimalesPotreros" data-rutaAplicacion="<?php echo $ruta;?>" data-opcion="guardarManejoAnimalesPotreros" data-destino="detalleItem">
		...
		<fieldset>
			...
			<div data-linea="35">
				<label id='lProgramaPrediosLibres'>Están dentro del programa de predios libres:</label>
...
<div class="pestania">
	...
	<form id="nuevoAbortoBrucelosis" data-rutaAplicacion="<?php echo $ruta;?>" data-opcion="guardarAbortoBrucelosis" data-destino="detalleItem">
		...
		<fieldset>
			...
			<div>
				<button type="submit" class="mas" id="btnGuardarAbortos">Agregar</button>		
			</div>
		</fieldset>
...
<div class="pestania">
	...
	<form id="nuevaPruebaBrucelosisLeche" data-rutaAplicacion="<?php echo $ruta;?>" data-opcion="guardarPruebaBrucelosisLeche" data-destino="detalleItem">
		...
		<fieldset>
			...
			<div data-linea="90">
				<label id="lPruebasLaboratorioLeche">Pruebas de Laboratorio:</label>
					<select id="pruebasLaboratorioLeche" name="pruebasLaboratorioLeche" >
						<option value="">Pruebas....</option>
						<option value="1">MilkRing Test MRT</option>
						<option value="2">ELISA indirecto en leche</option>
						
					</select>
					
					<input type="hidden" id="nombrePruebasLaboratorioLeche" name="nombrePruebasLaboratorioLeche"  /> 					
			</div>
			
			<div data-linea="91">
				<label id="lLaboratorioLeche">Laboratorio:</label>
					<select id="laboratorioLeche" name="laboratorioLeche">
			...
			<div data-linea="91">
				<input type="text" id="nombreLaboratorioLeche" name="nombreLaboratorioLeche"  /> 					
			</div>
			...
			<?php 
				while ($infoPruebasLeche = pg_fetch_assoc($pruebasLeche)){
					echo $cbt->imprimirLineaPruebaBrucelosisLecheCertificacionBT($infoPruebasLeche['id_certificacion_bt_prueba_brucelosis_leche'], 
												...
												$infoPruebasLeche['num_inspeccion'], $infoPruebasLeche['pruebas_laboratorio'], $infoPruebasLeche['laboratorio']);
				}
			?>
		</table>
	</fieldset>
			...
			<div data-linea="50">
				<label>Pruebas brucelosis en sangre:</label>
					<select id="pruebasBrucelosisSangre" name="pruebasBrucelosisSangre" required="required" >
						<option value="">Brucelosis Sangre....</option>
						<option value="Si">Si</option>
						<option value="No">No</option>
			...
			<?php 
				while ($infoPruebasSangre = pg_fetch_assoc($pruebasSangre)){
					echo $cbt->imprimirLineaPruebaBrucelosisSangreCertificacionBT($infoPruebasSangre['id_certificacion_bt_prueba_brucelosis_sangre'], 
														...
														$infoPruebasSangre['num_inspeccion']);
				}
			?>
		</table>
	</fieldset>
...
var perfil= <?php echo json_encode($perfilAdmin); ?>;
	...
	$("document").ready(function(){
		...
		$("#lPruebasLaboratorioLeche").hide();
		$("#pruebasLaboratorioLeche").hide();
		$("#lLaboratorioLeche").hide();
		$("#laboratorioLeche").hide();
		$("#nombreLaboratorioLeche").hide();
...
//                $("#btnGuardarAbortos").hide();
	...
	$("#pruebasBrucelosisLeche").change(function(){
		if($("#pruebasBrucelosisLeche option:selected").val()=='Si'){
    		...
    		$("#lPruebasLaboratorioLeche").show();
    		$("#pruebasLaboratorioLeche").show();
    		$("#pruebasLaboratorioLeche").attr('required','required');

    		$("#lLaboratorioLeche").show();
    		$("#laboratorioLeche").show();
    		$("#laboratorioLeche").attr('required','required');
    	}else{
    		$("#lResultadoBrucelosisLeche").hide();
    		$("#resultadoBrucelosisLeche").hide();
    		...
    		$("#lPruebasLaboratorioLeche").hide();
    		$("#pruebasLaboratorioLeche").hide();
    		$("#pruebasLaboratorioLeche").removeAttr('required');
    		$("#resultadoBrucelosisLeche").val('No Aplica');

    		$("#lLaboratorioLeche").hide();
    		$("#laboratorioLeche").hide();
    		$("#laboratorioLeche").removeAttr('required');
    		$("#laboratorioLeche").val('No Aplica');
    		$("#nombreLaboratorioLeche").val('No Aplica');
    	}
	});

	$("#pruebasLaboratorioLeche").change(function(){
    	$("#nombrePruebasLaboratorioLeche").val($("#pruebasLaboratorioLeche option:selected").text());
	});

	$("#laboratorioLeche").change(function(){		
        if($("#laboratorioLeche option:selected").val()!='0'){
        	$('#nombreLaboratorioLeche').hide();
    		$("#nombreLaboratorioLeche").val($("#laboratorioLeche option:selected").text());
        }else{
        	$("#nombreLaboratorioLeche").val('');
    	    $('#nombreLaboratorioLeche').show();
        }
	});
