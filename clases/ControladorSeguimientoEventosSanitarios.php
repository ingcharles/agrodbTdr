...
class ControladorProgramacionPresupuestaria{
	...
	public function listarNotificacionEventosSanitarios ($conexion, $provincia){
	...
	public function imprimirLineaPresupuesto($idPresupuesto, $actividad, $detalleGasto, $renglon, $costo, $cantidadAnual, $cuatrimestre, 
		...
		return '<tr id="R' . $idPresupuesto . '">' .
				...
				'<td>';
				if($estado=='creado'){
				echo '<form class="borrar" data-rutaAplicacion="'.$ruta.'" data-opcion="eliminarPresupuesto" data-destino="detalleItem" data-accionEnExito="NADA">' .
						'<input type="hidden" name="idPresupuesto" value="' . $idPresupuesto . '" >' .
						'<button type="submit" class="icono"></button>'.
					  '</form>'; 
				}
	...
	//Cerrar Proceso DGPGE
	public function cerrarProcesoPlanificacionAnualXItem ($conexion, $identificador, $estado, $estadoActual, $idPlanificacionAnual){
		...
		$res = $conexion->ejecutarConsulta("UPDATE
											...
											WHERE
												identificador_aprobador = '$identificador' and
												estado = '$estadoActual' and
												id_planificacion_anual = $idPlanificacionAnual;");
	
		return $res;
	}
	
	public function listarProgramacionAnualVistaAprobacionCierre ($conexion, $anio, $identificadorAprobador, $estado){
