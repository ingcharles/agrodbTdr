	...
	try{
		...
		try {
			...
			if(($identificador != null) || ($identificador != '')){
				...
				$veterinario = $cbt->buscarVeterinarioCertificacionBT($conexion, $idCertificacionBT, $numInspeccion);
		
				if(pg_num_rows($veterinario) == 0){
					$conexion->ejecutarConsulta("begin;");
					
					$idVeterinario = pg_fetch_result($cbt->guardarVeterinarioCertificacionBT($conexion, $idCertificacionBT, $identificador,
																...
																0, 'id_certificacion_bt_veterinario');				
					
					$mensaje['estado'] = 'exito';
					$mensaje['mensaje'] = $cbt->imprimirLineaVeterinarioCertificacionBT($idVeterinario, $nombreVeterinario, $telefonoVeterinario, 
