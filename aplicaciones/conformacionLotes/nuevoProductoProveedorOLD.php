	...
	<fieldset>
		...
		<div data-linea="2">
			...
			<select id="productos" name="productos">
					...
					$productos = $cl->listarProductosTrazabilidad($conexion,$operador);
...
$("#productos").change(function(event){
	...
	if($.trim($("#productos").val())!=""){
       ...
       /* $("#nuevoProductoProveedor").attr('data-destino', 'resultadoVariedad');
        $("#nuevoProductoProveedor").attr('data-opcion', 'comboProveedor');
        $("#opcion").val('variedad');
       	abrir($("#nuevoProductoProveedor"), event, false);*/
...
$("#nuevoProductoProveedor").submit(function(e) {
	...
	/*if(str!=""){
		...
		if(index>=0){			
			...
			if ($.trim($("#detalleItem #variedad").val()) == "" ) {
		        error = true;
		        $("#detalleItem #variedad").addClass("alertaCombo");
		        $("#estado").html("Por favor revise los campos obligatorios.").addClass('alerta');
		    }
		}
	}   */     
