	...
	try{
		...
		try {
			...
			if(($identificador != null) || ($identificador != '')){
				...
				$adquisicionAnimal = $cbt->buscarAdquisicionAnimalesRecertificacionBT($conexion, $idRecertificacionBT, $idProcedenciaAnimales, $idCategoriaAnimalesAdquiere, $numInspeccion);
		
				if(pg_num_rows($adquisicionAnimal) == 0){
					$conexion->ejecutarConsulta("begin;");
					
					$idAdquisicionAnimal = pg_fetch_result($cbt->guardarAdquisicionAnimalesRecertificacionBT($conexion, $idRecertificacionBT,
																	$identificador, $idProcedenciaAnimales, $procedenciaAnimales, 
																	$idCategoriaAnimalesAdquiere, $categoriaAnimalesAdquiere,
																	$numInspeccion), 
																	0, 'id_recertificacion_bt_adquisicion_animales');				
					
					$mensaje['estado'] = 'exito';
					$mensaje['mensaje'] = $cbt->imprimirLineaAdquisicionAnimalesRecertificacionBT($idAdquisicionAnimal,  
