...
$ext   = '.xls';
$nomReporte = 'gerencial_RIP_'.$fecha.$ext;
...
<body>
	<div id="header">
		...
		<div id="textoPOA" style="font-size: 16px; font-weight: bold;">
			...
			<br>AGROCALIDAD
                  <br>MATRIZ DE SERVICIOS DE DOSSIER PECUARIOS
		</div>
...
while ($registro = pg_fetch_assoc($res)) {
	...
	//N=Completado, S=Solicitado, R=Retornado, O=Observado, A=Aprobado
	if($registros['decision']!=null){
		...
		}else if($registros['decision']=='R'){
		    $registros['decision']='RETORNADO';
		}else if($registros['decision']=='S'){
		    $registros['decision']='SOLICITADO';
		}else if($registros['decision']=='N'){
		    $registros['decision']='COMPLETADO';
		}
