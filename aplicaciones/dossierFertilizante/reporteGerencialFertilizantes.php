...
$ext   = '.xls';
$nomReporte = 'gerencial_RIAF_'.$fecha.$ext;
...
$res = $cf->obtenerMatrizServicioFertilizantes($conexion);
...
<body>
	<div id="header">
		...
		<div id="textoPOA" style="font-size: 16px; font-weight: bold;">
			...
			<br>AGROCALIDAD
                  <br>MATRIZ DE SERVICIOS DE DOSSIER DE FERTILIZANTES
		</div>
...
while ($registro = pg_fetch_assoc($res)) {
	...
	if($registros['decision']!=null){
		...
		else if($registros['decision']=='T'){
			$registros['decision']='ETIQUETAS';
		}
		else if($registros['decision']=='S'){
			$registros['decision']='SOLICITUD';
		}
	}
