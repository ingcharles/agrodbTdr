...
$areaCoordinador = pg_fetch_assoc($ca->areaUsuario($conexion, $_SESSION['usuario']));
...
<header>
	...
	<nav>
		<form id="filtrar" action="aplicaciones/poa/reporteMPOAConsolidadoFiltrados.php" target="_blank" method="post">
			<input type="hidden" id="coordinador" name="coordinador" value="1" />
			...
			<table class="filtro">
				<tr>
					...
					<td><select name="areaDireccion" id="areaDireccion">
							<?php										
								echo '<option value="' . $areaCoordinador['id_area'] . '">' . $areaCoordinador['nombre'] .'</option>';
							?>
					</select></td>
				...
				<!-- tr>
					<th>En los</th>
					<td>estados:</td>
					<td><select name="estadoFiltro" id="estadoFiltro">
							<option value="1">Creado</option>
							<option value="2">Revisión Coordinador</option>
							<option value="3">Aprobado Coordinador</option>
							<option value="4">Aprobado en Planta Central</option>
						</select>
					</td>
					<td></td>
					<td></td>
