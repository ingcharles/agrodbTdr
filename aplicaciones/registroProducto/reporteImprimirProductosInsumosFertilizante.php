...
$ext   = '.xls';
$nomReporte = "REPORTE RPIF".$fecha.$ext;
...
$rpif = new ControladorCatalogos();


$res = $rpif-> reporteImprimirProductosInsumosFertilizantes($conexion);
	...
	echo '<tr>';
		...
		echo'<td>';
			foreach ((array)$registros['usos'] as $usos) { 
				...
				$usoss.=$usos['nombre_uso'].' '.($usos['nombre_especie'] != '' ? ' Aplicado a '. $usos['nombre_especie'] : ($usos['producto_aplicado'] != '' ? ' Aplicado a '. $usos['producto_aplicado'] : '')).', ';
			}
			echo rtrim ($usoss, ' ,' );
		echo '</td>';
		...
		echo '<td>'.$registros['observacion'].'</td>
              <td>'.$registros['nombre_empleado'].'</td>
