...
<body>
<header>
		<h1>Asignación de técnicos para análisis de ensayos</h1>
		<nav>
		<?php 
	...
	<div id="verificacionProtocolo">
		<h2>Solicitudes de protocolo por asignar técnico</h2>
		<div class="elementos"></div>
	</div>
	...
	<div id="verificacionInforme">
		<h2>Informes finales en revisión</h2>
		<div class="elementos"></div>
	</div>
	...
	$estadoInspeccion='verificacionInforme';
	$flujo=$ce->obtenerFlujo($conexion,$_SESSION['idAplicacion'],$estadoInspeccion);
	$faseInspeccion=$flujo['id_fase'];
	...
	if($perfil->tieneEstePerfil($perfilResponsable)){
		...
		//asignacion de informes finales
		$ts=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$faseInspeccion,$estadoInspeccion,$perfilResponsable,'N',true);
		while($fila = pg_fetch_assoc($ts))
		{
			$tramites[]=$fila;
		}
		$perfiles[]=$perfilResponsable;
	}
	...
	if($perfil->tieneEstePerfil($perfilDirector)){
		...
		//asignacion de informes finales
		$ts=$ce->obtenerFlujosInformesFinalesAsignarEE($conexion,$identificador,$faseInspeccion,$estadoInspeccion,$perfilDirector,'N');
		while($fila = pg_fetch_assoc($ts))
		{

			$tramites[]=$fila;
		}
		$perfiles[]=$perfilDirector;
	}
		...
		foreach($tramites as $key=>$fila)
		{
			...
			if($fechaTiempo!=null){
				...
				$contenido = '<article
							...
							data-idOpcion="'.$fila['tipo_documento'].'"
							data-nombre="'.$fila['id_tramite_flujo'].'"
							class="item"
							data-rutaAplicacion="ensayoEficacia"
							data-opcion="abrirAsignacionTecnicos"
