<?php 
	...
	$presupuesto = $crp->listarPresupuestosTemporalesXEstado($conexion, $idProgramacionAnual,"enviadoRevisorGF");
	
	$totalMonto = pg_fetch_result($crp->numeroPresupuestosYCostoTotalTemporal($conexion, $idProgramacionAnual), 0, 'total');
	/*$total = pg_fetch_assoc($crp->numeroPresupuestosReformadosTemporal($conexion, $idPlanificacionAnual, "enviadoRevisorGF"));
	$presupuestosRevisados = pg_fetch_result($crp->numeroPresupuestosRevisadosTemporal($conexion, $idProgramacionAnual, 'revisadoGA'), 0, 'num_presupuestos_revisados');
	...
	$presupuestosEnviadosRevisor = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $idProgramacionAnual, 'enviadoRevisorGF'), 0, 'num_presupuestos_revisados');
				...
				<?php 
					while ($presupuestos = pg_fetch_assoc($presupuesto)){
						echo $crp->imprimirLineaPresupuestoRevisionDGAFDGPGE($presupuestos['id_presupuesto'], $presupuestos['nombre_actividad'], $presupuestos['detalle_gasto'], 
															...
															$presupuestos['revisado'],'GF', $presupuestos['estado']);
					}
				?>
			</table>
		</fieldset>
