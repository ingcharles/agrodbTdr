...
<header>
	...
	$estado='analizarDossier';
	$flujo=$ce->obtenerFlujo($conexion,$_SESSION['idAplicacion'],$estado);

	$yaDesplego=false;
	$tramites =array();
	$perfilRIP='PFL_DG_ARIA';
	if($perfil->tieneEstePerfil($perfilRIP)){
		$ts=$cg->obtenerFlujosDeTramitesParaAsingnarDG($conexion,$identificador,$flujo['id_fase'],'N');
		...
		foreach($tramites as $key=>$fila)
		{
			...
			if($fechaTiempo!=null){
				...
				$contenido = '<article
							...
							data-opcion="abrirAnalizarDossier"
