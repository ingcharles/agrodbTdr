...
try{
	...
	$datos = array(	'vehiculo' => htmlspecialchars ($_POST['vehiculo'],ENT_NOQUOTES,'UTF-8'),
	...
	try {
		...
		if ($identificadorUsuarioRegistro != ''){
			...
			$qLavado = $cv -> guardarNuevoMantenimiento($conexion,$numero, 'Lavada de vehículo', $datos['vehiculo'], $datos['conductor'], $datos['kilometraje'], $datos['taller'],$_SESSION['nombreLocalizacion'],'Lavada', $datos['idVehiculo'], $identificadorUsuarioRegistro);
			$idLavado = pg_fetch_result($qLavado, 0, 'id_mantenimiento');
			//$cv ->actualizarEstadoVehiculo($conexion, $datos['vehiculo'], 'Lavado');
