...
?>
		...
		<nav>
		<form id="filtrar" data-rutaAplicacion="uath" data-opcion="listaExperienciaLaboralAdmin" data-destino="areaTrabajo #listadoItems">
		...
		<table>
			...
			<?php 
				...
				$res = $cd->obtenerExperienciaLaboralAdmin($conexion, $identificador,$apellido,$nombre);
				$contador = 0;
				...
				if($areaUsuarioTH['clasificacion']=='Planta Central'){
					...
					{
						echo '<tr 	id="'.$experiencia['identificador'].'"
									class="item"
									data-rutaAplicacion="uath"
									data-opcion="revisarExperienciaLaboralAdmin" 
									ondragstart="drag(event)" 
									draggable="true" 
									data-destino="detalleItem"
									>
								<td>'.++$contador.'</td>
								<td style="white-space:nowrap;"><b>'.$experiencia['identificador'].'</b></td>
								<td>'.$experiencia['apellido'].'</td>
					...
					while($experiencia = pg_fetch_assoc($res)){
						if (in_array($experiencia['id_area'], $zonasFuncionarios)) {
							echo '<tr 	id="'.$experiencia['identificador'].'"
									class="item"
									data-rutaAplicacion="uath"
									data-opcion="revisarExperienciaLaboralAdmin" 
