...
try{
	...
	try {
		...
		$cantidadSubItems = $cac->obtenerSubItemsPorIdentificador($conexion, $idSubitemCatalogoPadre);
		
		if(pg_num_rows($cantidadSubItems) == 0){
		    
		    $cac->inactivarSubItem($conexion, $idSubitemCatalogoPadre, $identificador);
		    
		    $mensaje['estado'] = 'exito';
		    $mensaje['mensaje'] = $idCatalogoPadre .$idCatalogoHijo.$idItemPadre.$idItemHijo;
		}else{
