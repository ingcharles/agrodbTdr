...
switch ($opcion){
	...
	break;
	case 'puertos':
		$embarque=$cc->listarPuertosPorPais($conexion, $idPais);
		echo '<label for="puertoEmbarque">*Puerto de Embarque:</label>
		...
		while($fila=pg_fetch_assoc($embarque)){
			...
			echo '</select>
			<input type="hidden" id="nombrePuerto" name="nombrePuerto" value="'.$fila['nombre_puerto'].'">';
	break;
}
...
$("#subTipoProducto").change(function(event){
	...
	if($("#subTipoProducto").val()!=""){
		...
		$("#nuevoRegistro #opcion").val("producto");
...
function agregar(){
	...
	if(!error){
		if($("#bodyTablaProductos #r_"+idProducto+identificadorProductoReplace.toString()).length==0){
			var cadena= '<tr id="r_'+idProducto+identificadorProductoReplace+'"><td><input type="hidden" name="didTipoProducto[]" value="'+idTipoPorducto+'" style="width:30px">'+
								...
								+nTipoPorducto+'</td>'+
								'<td>'+nProducto+ '<input type="hidden" value="'+idProducto+'">'+ ' </td>'+
								'<td>'+identicacionProducto+'</td>'+
