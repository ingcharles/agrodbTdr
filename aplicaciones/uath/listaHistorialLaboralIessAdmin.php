...
?>
<header>
			<h1>Datos Historial Laboral IESS</h1>
			<nav>
			
		</nav>
		<nav>
		<form id="filtrar" data-rutaAplicacion="uath" data-opcion="listaHistorialLaboralIessAdmin" data-destino="areaTrabajo #listadoItems">
		...
		<table>
			...
			<?php 
				...
				$res = $cd->obtenerHistorialLaboral($conexion,$identificador,$apellido,$nombre);
				$contador = 0;
				...
				if($areaUsuarioTH['clasificacion']=='Planta Central'){
					while($capacitacion = pg_fetch_assoc($res)){
						echo '<tr id="'.$capacitacion['identificador'].'"
								...
								data-opcion="revisarHistorialLaboralIess" 
