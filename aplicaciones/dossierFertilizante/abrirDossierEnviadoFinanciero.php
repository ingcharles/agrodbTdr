...
$datosGenerales=$cdf->obtenerSolicitud($conexion, $idSolicitud);
...
if($condicion == 'verificacion' || $condicion == 'verificacionVUE'){	
	$qOrdenPago=$cce->obtenerIdOrdenPagoXtipoOperacionSinGrupo($conexion, $idSolicitud , 'dossierFertilizantes');	
}

if($condicion == 'pago'){
    echo '<input type="hidden" class= "abrirPago" id="'.$idSolicitud.'-'.$operador['identificador'].'-'.'pago'.'-dossierFertilizantes-tarifarioNuevo'.'" data-rutaAplicacion="financiero" data-opcion="asignarMontoSolicitudSinGrupo" data-destino="ordenPago" />';
}else if ($condicion == 'verificacionVUE' && pg_num_rows($qOrdenPago)!=0){
    echo '<input type="hidden" class= "abrirPago" id="'.$idSolicitud.'-'.$operador['identificador'].'-'.'verificacionVUE'.'-dossierFertilizantes-tarifarioNuevo-'.'" data-rutaAplicacion="financiero" data-opcion="asignarMontoSolicitudSinGrupo" data-destino="ordenPago" />';
}else if ($condicion == 'verificacion' && pg_num_rows($qOrdenPago)!=0){
    echo '<input type="hidden" class= "abrirPago" id="'.$idSolicitud.'-'.$operador['identificador'].'-'.'verificacion'.'-dossierFertilizantes-'.pg_fetch_result($qOrdenPago, 0, 'id_pago').'" data-rutaAplicacion="financiero" data-opcion="finalizarMontoSolicitudSinGrupo" data-destino="ordenPago" />';
}
...
<div id="estado"></div>
      ...
      <fieldset>
		...
		<div data-linea="11">
			...
			<?php
				   $tipoSolicitud = $cee->obtenerItemDelCatalogo($conexion,'P3C15', $datosGenerales['tipo_producto']);
		...
		<div data-linea="5">
         ...
         <hr>
         <div data-linea="7">
            <label for="nombreProducto">Nombre del producto: </label><?php echo $datosGenerales['producto_nombre'];?>
         </div>
         <div data-linea="8">
			<label for="id_subtipo_producto">Objetivo: </label>
				<?php 
				    $tipoSolicitud = $cee->obtenerItemDelCatalogo($conexion,'P3C13', $datosGenerales['objetivo']);
				    echo $tipoSolicitud['nombre'];
				?>	
		</div>
		<div data-linea="9">
