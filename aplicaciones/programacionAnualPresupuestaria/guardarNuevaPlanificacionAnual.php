<?php 
	...
	$qIdRevisor = $ca->buscarResponsableSubproceso($conexion,$idAreaRevisor);
	
	if(pg_num_rows($qIdRevisor)==0){
	    $idAreaPadre = pg_fetch_assoc($ca->buscarPadreSubprocesos($conexion, $idAreaRevisor));
	    $qIdRevisor = $ca->buscarResponsableSubproceso($conexion,$idAreaPadre['id_area_padre']);
	    $idAreaRevisor = $idAreaPadre['id_area_padre'];
	}
	
	$idRevisor = pg_fetch_result($qIdRevisor, 0, 'identificador');		
