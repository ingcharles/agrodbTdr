	...
	if ($parametro['proveedores']=="1" ){
	   ...
	   // $proveedorLote = pg_fetch_assoc( $cl->obtenerProveedorLote($conexion, $idLote));
	    ...
	    if($parametro['areas_proveedor']!="3"){
	       ...
	       $producto = $cl->sitiosXidProductoAcopiador($conexion, $filaRegistro['id_producto'],$filaRegistro['identificador_proveedor'],$parametro['areas_proveedor'],$usuario);
	    
	    echo "<div data-linea=7 id=resultadoSitioProveedor>
        <label for=sitioProveedor>Área Proveedor:</label>
		   ...
		   var arrayConformar = [];
            ...
            arrayConformar.forEach(function(element){
                if(element.toUpperCase() =='VARIEDAD'){
                	...
                	variedad = $('#tablaLotesConformar tbody tr:eq(0) td:eq('+i+')').text();
			    		...
			    		if(rDuplicado==1){			    			
							...
							if($("#CAreaProveedor").val()=="1"){
    						    ...
    						    if(!validarAreaUnica){
    						    	...
    						    	mostrarMensaje("No puede conformar un lote con diferentes áreas del proveedor.","FALLO");
    						    }
