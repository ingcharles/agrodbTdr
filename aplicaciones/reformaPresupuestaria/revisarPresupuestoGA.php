	...
	try {
		...
		if ($idRevisor != ''){
			$conexion->ejecutarConsulta("begin;");
				$crp -> aprobarPresupuestoTemporalGA($conexion, $idPresupuesto, $estado, $observaciones, $idRevisor);
				...
				$crp -> registrarControlCambios($conexion, $idPlanificacionAnual, $idPresupuesto, $numControlCambios,
						$identificador, $idAreaFuncionario, 'Revisión de presupuesto por Gestión Administrativa - DGAF', $detalleGasto,
				...
				$presupuestosRevisados = pg_fetch_result($crp->numeroPresupuestosRevisadosTemporal($conexion, $idPlanificacionAnual, 'revisadoGA'), 0, 'num_presupuestos_revisados');
				...
				$presupuestosEnviadosRevisor = pg_fetch_result($crp->numeroPresupuestosXEstadoTemporal($conexion, $idPlanificacionAnual, 'enviadoRevisorGA'), 0, 'num_presupuestos_revisados');
				...
				if(($total['num_presupuestos'] - $presupuestosAprobados) == $presupuestosRevisados){
					$crp -> enviarPlanificacionAnualTemporal($conexion, $idPlanificacionAnual, 'revisadoGA');
					
					//Ingresar fecha de revision y Observaciones
					$crp -> actualizarFechaRevisionReformaPlanificacionAnualTemporal($conexion, $idPlanificacionAnual, 'GA', 'revisado');
				}
