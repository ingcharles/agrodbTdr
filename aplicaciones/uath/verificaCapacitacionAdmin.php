...
try{
	...
	$capacitacion_seleccionado=$_POST['capacitacion_seleccionado'];
	$n        = count($estado);
	$i        = 0;
		...
		try {
				...
				while ($i < $n)
				{
					$cc->verificaCapacitacionAdmin($conexion, $capacitacion_seleccionado[$i], $estado[$i], $observaciones[$i]);
					if(strcmp($estado[$i],'Rechazado') == 0 ){
						$fila=pg_fetch_assoc($cc->obtenerDatosUsuarioAgrocalidad($conexion,$_POST['identificadorServidor']));
						$asunto = 'VALIDACIÓN DE INFORMACIÓN EN CAPACITACIÓN';
						...
						if($mailDestino != ''){
							...
							$tablaModulo = 'g_uath.datos_capacitacion';
					
							$qGuardarCorreo=$cMail->guardarCorreo($conexion, $asunto, $cuerpoMensaje, 'Por enviar', $codigoModulo, $tablaModulo, $capacitacion_seleccionado[$i]);
