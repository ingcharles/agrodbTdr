...
$qPuertoEcuador = $cc -> listarPuertosEcuador($conexion);
...
<header>
	...
	<form id='nuevaFitosanitario' data-rutaAplicacion='fitosanitario' data-opcion='comboPuertos' data-destino="comboPuertoEmbarque" data-accionEnExito="ACTUALIZAR">
			...
			<legend>Detalle de Productos</legend>
				...
				<div data-linea="17">	
					...
					<select id="unidades" name="unidades">
						<option value="">Seleccione........</option>
							<?php
							    $unidadMedida = $fi->listarUnidadesMedida($conexion);
							    while ($fila = pg_fetch_assoc($unidadMedida)){
							    	echo '<option value="' . $fila['id_unidades_medidas'] . '">' . $fila['nombre'] . '</option>';
							    }
