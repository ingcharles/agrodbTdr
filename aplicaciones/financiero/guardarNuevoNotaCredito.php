	...
	try{
		...
		$datosInstitucion = pg_fetch_assoc($institucion);
			...
			if($idNotaCredito==""){
				...
				$cf->guardarUsoDetalleFactura($conexion,$idPago, $identificadorUsuario, $provincia, $idArea, 'Factura consumida por emisión de nota de crédito # '.$datosInstitucion['numero_establecimiento'].'-'.$datosInstitucion['punto_emision'].'-'.$numeroNotaCredito);
