...
<body>
<header>
		<h1>Evaluación del método analítico</h1>
		<nav>
		<?php 
	...
	$perfilTecnico='PFL_DP_TL';
	if($perfil->tieneEstePerfil($perfilTecnico)){

		$ts=$cp->obtenerFlujosDeTramitesCentralesParaAsingnarTecnicoDP($conexion,$identificador,$flujo['id_fase'],$identificador,'N');
		while($fila = pg_fetch_assoc($ts))
		{
			$tramites[]=$fila;
		}
		...
		foreach($tramites as $key=>$fila)
		{
			...
			if($fechaTiempo!=null){
				...
				$contenido = '<article
							...
							data-opcion="abrirEvaluarMetodo"
