...
$mensaje['mensaje'] = 'Ha ocurrido un error!';
	...
	$certificacion = htmlspecialchars ($_POST['certificacion'],ENT_NOQUOTES,'UTF-8');
		...
		if(($identificador != null) || ($identificador != '')){
			...
			$cbt->modificarCertificacionBT($conexion, $idCertificacionBT, $identificador, $fecha, 
											$nombreEncuestado, $telefonoPropietario, $celularPropietario, 
											$correoElectronicoPropietario, $numCertFiebreAftosa, $x, $y, 
											$z, $huso);
		...
		switch ($certificacion){
			case 'Brucelosis':
				if($estado == 'inspeccion'){
					echo '<input type="hidden" id="' . $idCertificacionBT . '" data-rutaAplicacion="certificacionBrucelosisTuberculosis" data-opcion="abrirCertificacionBTBrucelosisInspeccion" data-destino="detalleItem"/>';
				}else{
					echo '<input type="hidden" id="' . $idCertificacionBT . '" data-rutaAplicacion="certificacionBrucelosisTuberculosis" data-opcion="abrirCertificacionBTBrucelosis" data-destino="detalleItem"/>';
				}
				break;
		
			case 'Tuberculosis':
				if($estado == 'inspeccion'){
					echo '<input type="hidden" id="' . $idCertificacionBT . '" data-rutaAplicacion="certificacionBrucelosisTuberculosis" data-opcion="abrirCertificacionBTTuberculosisInspeccion" data-destino="detalleItem"/>';
				}else{
					echo '<input type="hidden" id="' . $idCertificacionBT . '" data-rutaAplicacion="certificacionBrucelosisTuberculosis" data-opcion="abrirCertificacionBTTuberculosis" data-destino="detalleItem"/>';
				}
				break;
