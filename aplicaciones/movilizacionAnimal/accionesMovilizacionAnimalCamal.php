...
for($i=0; $i<count($idProducto); $i++){//inicio for
	...
	$idMovilizacionAnimalDetalle = pg_fetch_result($movilizacionAnimalDetalle, 0, 'id_movilizacion_animal_detalle');
	...
	$movientoOrigen = array(
			...
			,'id_concepto_catastro' => 12 //'Movimiento camal'
	...
	if($id_producto_condicion==0){//si es diferente de lechon, que no se vacuna	
		...
		//echo "...Movimiento catastro-resta para vacunados -- fin";
	}
//	echo "...Movimiento catastro-resta fin";
	
	//Movimiento destino (suma) a Camal con motivo de muerte animal 
	$idCoeficiente = 1;
	$movientoDestino = array(
			...
			,'id_especie' => $detalle['id_especie']
			,'nombre_especie' => $detalle['nombre_especie']
			,'id_concepto_catastro' => 7 // => Muerte del animal
			,'numero_documento' => $detalle['numero_certificado']
			,'edad_producto' => $detalle['edad_producto']//fecha de nacimiento del producto
			,'id_producto' => $detalle['id_producto']
			,'coeficiente' => $idCoeficiente //Movimiento destino (suma)
	...
	if($id_producto_condicion==0){//si es diferente de lechon, que no se vacuna
		...
		while ($fila = pg_fetch_assoc($controlVD)){
			$id_concepto_catastro = 12;//"Vacunación destino"10
			$saldo = $fila['total'];
			$fecha_vacunacion = $fila['fecha_vacunacion'];
		}
