...
<header>
	...
	$contador = 0;
	while($fila = pg_fetch_assoc($res)){
	$verificar= pg_fetch_result($ced->verificarEvaluacionesServidores($conexion, $_SESSION['usuario'],$fila['id_evaluacion']), 0, 'valor') ;
	if($verificar !=0)
		echo '<article
						id="'.$fila['id_evaluacion'].'"
						class="item"
						data-rutaAplicacion="evaluacionesDesempenio"
						data-opcion="abrirResultadoEvaluacion"
