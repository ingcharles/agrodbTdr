...
<body>
	...
	<div id="IAP">
		<h2>Registro de insumos agrícolas</h2>
	...
	switch ($opcion){
		case 'subTipoProducto':
			$res = $cr->listarRequisitosComercializacionProducto($conexion,	$tipoProducto,'tipoProducto');
		break;
		
		case 'producto':
			$res = $cr->listarRequisitosComercializacionProducto($conexion,$subTipoProducto, 'subTipoProducto');
		break;
		
		default:
			$res = $cr->listarRequisitosComercializacionProducto($conexion,$producto, 'producto');
	}
		...
		while($fila = pg_fetch_assoc($res)){
			...
			$contenido = '<article 
					...
					<aside><small>'.$fila['id_area'].'</small></aside>			
				</article>';
			?>
